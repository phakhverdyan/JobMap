...
/*    if (window.signup_js_initialized) {
    ...
    new GraphQL("query", "languages", {}, [
        ...
        if (data) {
            ...
            $('#setting-languages-list_signup').html(html);
        }
    }, false).request();

    $('body').on('click', '.item-language', function () {
        $('#setting-languages-list_signup').find('button').removeClass('active');
    ...
    $('.user__signup').click(function () {
        $('#signUpModal').modal('show');
    });

    //---start-now registration
    $('#start-now').click(function () {
        var form = $('#form-start-now'),
        ...
        if (firstName.length < 1) {
            error['first_name'] = 'The first name field is required.'
            isValid = false;
        }
        if (lastName.length < 1) {
            error['last_name'] = 'The last name field is required.'
            isValid = false;
        }
        if (isValid) {
            /*APIStorage.create('f_name',firstName);
            APIStorage.create('l_name',lastName);*/
            signUpForm.find('input[name="first_name"]').val(firstName);
            signUpForm.find('input[name="last_name"]').val(lastName);
            ...
            FormValidate.fieldsValidate(error,form);
        }
    });
    $('#form-start-now').on('click', 'input', function () {
        FormValidate.fieldValidateClear($(this));
    });
