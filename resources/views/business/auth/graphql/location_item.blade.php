<div class="card border-0 location-item" data-item-id="{{ $args['id'] }}">
            ...
            <a class="collapsed Bobik" data-toggle="collapse" href="#collapse{{ $args['id'] }}"
               ...
               style="font-size: 16px; font-family: sans-regular; color: #4E5C6E;">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                     ...
                     style="enable-background:new 0 0 57.502 57.502; fill:#7b7b7b; vertical-align: middle; margin-top: -3px;"
                     xml:space="preserve" width="20px" height="20px" class="mr-2"><g>
                        <g>
                ...
                {{ $args['localized_name'] }}@if($args['main'] == 1) {!! trans('main.status.main_location') !!} @endif
            </a>

            <div class="btn-group float-right mr-3" role="group">
                <button id="btnGroupDrop1-{{ $args['id'] }}" type="button"
                ...
                <div class="dropdown-menu" aria-labelledby="btnGroupDrop1-{{ $args['id'] }}">
                    <button class="dropdown-item QRCodeFunc-show-action"
                            type="button"
                            role="button"
{{--                            onclick="modals.location_qr_code({ location: JSON.parse(atob($(this).attr('data-json'))) });"--}}
                            data-json="{{ base64_encode(json_encode($args)) }}"
                    >
                        {!! trans('main.buttons.job_map_scanner') !!}
                    </button>
{{--                    <button class="dropdown-item business-manager-assign" type="button" role='button'--}}
{{--                            data-toggle="modal" data-target="#ManaInLocModal" data-id="{{ $args['id'] }}">--}}
{{--                        <span>{{ $args['managers_count'] }}</span> {!! trans('main.buttons.managers') !!}--}}
{{--                    </button>--}}
{{--                    <button class="dropdown-item business-job-status-assign" type="button" role='button'--}}
{{--                            data-toggle="modal" data-target="#OpenClosedModal" data-id="{{ $args['id'] }}">--}}
{{--                        <span>{{ $args['jobs_count_open'] }}</span> {!! trans('main.status.open') !!}--}}
{{--                        | <span>{{ $args['jobs_count_close'] }}</span> {!! trans('main.status.closed') !!}--}}
{{--                    </button>--}}
                    {{--<button class="dropdown-item" type="button" role='button'>0--}}
                        {{--candidates--}}
                    {{--</button>--}}
                    @if ($isEdit)
                        <a class="dropdown-item" href="{{ url('/business/branch/edit?id='.$args['id']) }}" role='button'>{!! trans('main.buttons.edit') !!}</a>
                    @endif
                    <button class="dropdown-item" type="button" data-toggle="modal"
                            data-target="#ShareModal" role='button' data-id="{{ $args['id'] }}"
                            data-title="{{ $args['localized_name'] }}"
                            data-link="{{ url('/map/view/location/' . $args['id'] . '/' . str_slug($args['localized_name'])) }}"
                            data-name="{{ str_slug($args['localized_name']) }}">{!! trans('main.buttons.share') !!}
                    </button>
                    @if ($isEdit)
                        <a class="dropdown-item business-location-clone" data-id="{{ $args['id'] }}"
                           href="{{ url('/business/branch/clone?id='.$args['id']) }}" role='button'>{!! trans('main.buttons.clone') !!}
                        </a>
                        <button class="dropdown-item business-location-delete" data-toggle="modal"
                                data-title="{{ $args['localized_name'] }}"
                                data-link="{{ url('/map/view/location/' . $args['id'] . '/' . str_slug($args['localized_name'])) }}"
                                data-target="#deleteModal" data-id="{{ $args['id'] }}" type="button" role='button'>{!! trans('main.buttons.delete') !!}
                        </button>
                    @endif
                </div>
            </div>
{{--            <a data-toggle="modal" data-target="#ShareModal" data-id="{{ $args['id'] }}" data-title="{{ $args['localized_name'] }}" data-link="{{ url('/map/view/location/' . $args['id'] . '/' . str_slug($args['localized_name'])) }}" class="float-right mr-3" data-name="{{ str_slug($args['name']) }}">--}}
...
{{--            </a>--}}
    ...
    <div id="collapse-{{ $args['id'] }}" class="" role="tabpanel" aria-labelledby="heading{{ $args['id'] }}" data-parent="#accordion--">
        <div class="card-body pt-0">
            <div class="col-12 px-0">
                <div class="d-flex">
                    <p class="MarksBeautifulFontColor mb-0">
                        <span><strong style="font-size: 16px;">{{ $args['street_number'].' '.$args['street'].', '.$location }}</strong></span>
                    </p>
                </div>
            </div>
        </div>
