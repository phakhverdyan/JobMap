<div class="card mb-5">
    <div class="row">
        <div class="col-md-12 text-center mt-4">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                 id="Layer_1" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;"
                 xml:space="preserve" width="50px" height="50px"><g>
                    <g>
                        <g>
                            <path d="M431.279,0H80.721c-5.633,0-10.199,4.566-10.199,10.199v491.602c0,5.633,4.566,10.199,10.199,10.199h266.562    c2.705,0,5.298-1.075,7.212-2.987l83.997-83.998c1.912-1.912,2.987-4.506,2.987-7.212V10.199C441.479,4.566,436.912,0,431.279,0z     M357.463,477.196l-0.044-49.257l49.257,0.045L357.463,477.196z M421.081,212.151c-5.565,0.08-10.053,4.609-10.053,10.192    c0,5.583,4.489,10.112,10.052,10.192v175.064l-73.862-0.067c-0.003,0-0.006,0-0.009,0c-2.705,0-5.298,1.075-7.212,2.987    c-1.914,1.915-2.989,4.513-2.987,7.221l0.067,73.862H90.92v-259.06h0.873c5.633,0,10.199-4.566,10.199-10.199    c0-5.633-4.566-10.199-10.199-10.199H90.92V20.398h330.161V212.151z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M325.318,66.347h-55.833c-5.632,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h55.833    c5.632,0,10.199-4.566,10.199-10.199C335.517,70.913,330.95,66.347,325.318,66.347z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M390.63,113.204H269.484c-5.632,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199H390.63    c5.632,0,10.199-4.566,10.199-10.199C400.829,117.77,396.261,113.204,390.63,113.204z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M390.63,160.128H269.484c-5.632,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199H390.63    c5.632,0,10.199-4.566,10.199-10.199C400.829,164.694,396.261,160.128,390.63,160.128z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M250.335,268.291H129.53c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h120.805    c5.633,0,10.199-4.566,10.199-10.199C260.535,272.857,255.968,268.291,250.335,268.291z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M391.649,309.543H129.53c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h262.12    c5.632,0,10.199-4.566,10.199-10.199C401.849,314.109,397.281,309.543,391.649,309.543z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M391.649,350.853H129.53c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h262.12    c5.632,0,10.199-4.566,10.199-10.199C401.849,355.419,397.281,350.853,391.649,350.853z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M239.681,421.227h-8.614c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h8.614    c5.633,0,10.199-4.566,10.199-10.199C249.88,425.793,245.314,421.227,239.681,421.227z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M195.825,421.227H129.53c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h66.295    c5.633,0,10.199-4.566,10.199-10.199C206.024,425.793,201.457,421.227,195.825,421.227z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M199.196,52.209c-5.223-5.574-12.599-8.771-20.237-8.771c-7.638,0-15.015,3.197-20.237,8.771    c-5.222,5.574-7.933,13.143-7.436,20.766l1.285,19.698c0.553,8.471,5.033,16.225,11.985,20.742    c4.374,2.843,9.389,4.263,14.403,4.263c5.014,0,10.029-1.421,14.403-4.263c6.953-4.517,11.433-12.272,11.985-20.742l1.285-19.698    C207.128,65.351,204.419,57.783,199.196,52.209z M186.276,71.647l-1.285,19.698c-0.136,2.081-1.161,3.935-2.743,4.963    c-1.999,1.298-4.581,1.297-6.581,0c-1.582-1.028-2.607-2.883-2.743-4.963l-1.285-19.698c-0.133-2.045,0.565-3.995,1.966-5.491    s3.302-2.319,5.352-2.319c2.05,0,3.95,0.824,5.352,2.319C185.711,67.651,186.41,69.601,186.276,71.647z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M244.543,169.528l-2.229-12.302c-3.089-17.054-16.601-30.666-33.624-33.872c-2.57-0.483-5.196-0.728-7.807-0.728h-6.712    c-2.705,0-5.299,1.075-7.212,2.987c-2.137,2.137-4.978,3.314-8,3.314c-3.022,0-5.864-1.177-8-3.314    c-1.912-1.912-4.507-2.987-7.212-2.987h-6.712c-2.611,0-5.237,0.245-7.809,0.729c-17.021,3.206-30.533,16.816-33.623,33.871    l-2.229,12.302c-0.539,2.975,0.269,6.035,2.208,8.356c1.937,2.32,4.804,3.661,7.827,3.661h111.097c3.023,0,5.89-1.341,7.828-3.661    C244.273,175.564,245.082,172.503,244.543,169.528z M135.624,161.147l0.051-0.285c1.593-8.793,8.556-15.81,17.325-17.461    c1.329-0.25,2.685-0.377,4.035-0.377h2.948c11.209,8.383,26.744,8.383,37.953,0h2.948c1.348,0,2.706,0.127,4.034,0.376    c8.77,1.651,15.733,8.669,17.326,17.462l0.052,0.285H135.624z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M357.875,212.143h-12.67c-5.632,0-10.199,4.566-10.199,10.199c0,5.633,4.567,10.199,10.199,10.199h12.67    c5.632,0,10.199-4.566,10.199-10.199C368.074,216.71,363.507,212.143,357.875,212.143z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M319.862,212.143h-12.67c-5.632,0-10.199,4.566-10.199,10.199c0,5.633,4.567,10.199,10.199,10.199h12.67    c5.632,0,10.199-4.566,10.199-10.199C330.062,216.71,325.494,212.143,319.862,212.143z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M129.804,212.143h-12.67c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h12.67    c5.633,0,10.199-4.566,10.199-10.199C140.003,216.71,135.437,212.143,129.804,212.143z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M243.84,212.143h-12.671c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h12.671    c5.633,0,10.199-4.566,10.199-10.199C254.039,216.71,249.473,212.143,243.84,212.143z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M205.828,212.143h-12.67c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h12.67    c5.633,0,10.199-4.566,10.199-10.199C216.027,216.71,211.461,212.143,205.828,212.143z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M395.886,212.143h-12.672c-5.632,0-10.199,4.566-10.199,10.199c0,5.633,4.567,10.199,10.199,10.199h12.672    c5.632,0,10.199-4.566,10.199-10.199C406.085,216.71,401.518,212.143,395.886,212.143z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M281.851,212.143h-12.67c-5.632,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h12.67    c5.632,0,10.199-4.566,10.199-10.199C292.05,216.71,287.483,212.143,281.851,212.143z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                    <g>
                        <g>
                            <path d="M167.817,212.143h-12.67c-5.633,0-10.199,4.566-10.199,10.199c0,5.633,4.566,10.199,10.199,10.199h12.67    c5.633,0,10.199-4.566,10.199-10.199C178.016,216.71,173.45,212.143,167.817,212.143z"
                                  data-original="#000000" class="active-path" data-old_color="#007bff"
                                  fill="#007bff"></path>
                        </g>
                    </g>
                </g>
                                                    </svg>
        </div>
        <div class="col-md-12 text-center pb-3">
            <h2 class="mx-auto mt-2 text-muted" style="font-family: 'Open Sans', sans-serif; letter-spacing: -1px">
                Overview</h2>
        </div>
    </div>


    <div class="card-header pb-4 pt-0 border border-left-0 border-right-0" style="background-color: #fff;">


        <div class="row justify-content-between">
            <div class="col-10 pt-4">
                <div class="text-left d-flex align-items-start">
                    <div class="p-4 mb-3 d-inline-block rounded bg-white mr-3 {{ $args['user_pic_filter'] }}"
                         style="box-shadow: 0 4px 10px 0 rgba(0, 0, 0, 0.14), 0 1px 2px 0 rgba(0, 0, 0, 0.12), 0 3px 1px -2px rgba(0, 0, 0, 0.2);">

                        {{--<svg version="1.1" id="Capa_1" width="50px" height="50px" xmlns="http://www.w3.org/2000/svg"  x="0px" y="0px" viewBox="0 0 482.9 482.9" style="enable-background:new 0 0 482.9 482.9;" xml:space="preserve">--}}
                        {{--<path d="M239.7,260.2c0.5,0,1,0,1.6,0c0.2,0,0.4,0,0.6,0c0.3,0,0.7,0,1,0c29.3-0.5,53-10.8,70.5-30.5--}}
                        {{--c38.5-43.4,32.1-117.8,31.4-124.9c-2.5-53.3-27.7-78.8-48.5-90.7C280.8,5.2,262.7,0.4,242.5,0h-0.7c-0.1,0-0.3,0-0.4,0h-0.6--}}
                        {{--c-11.1,0-32.9,1.8-53.8,13.7c-21,11.9-46.6,37.4-49.1,91.1c-0.7,7.1-7.1,81.5,31.4,124.9C186.7,249.4,210.4,259.7,239.7,260.2z--}}
                        {{--M164.6,107.3c0-0.3,0.1-0.6,0.1-0.8c3.3-71.7,54.2-79.4,76-79.4h0.4c0.2,0,0.5,0,0.8,0c27,0.6,72.9,11.6,76,79.4--}}
                        {{--c0,0.3,0,0.6,0.1,0.8c0.1,0.7,7.1,68.7-24.7,104.5c-12.6,14.2-29.4,21.2-51.5,21.4c-0.2,0-0.3,0-0.5,0l0,0c-0.2,0-0.3,0-0.5,0--}}
                        {{--c-22-0.2-38.9-7.2-51.4-21.4C157.7,176.2,164.5,107.9,164.6,107.3z" fill="#007bff"/>--}}
                        {{--<path d="M446.8,383.6c0-0.1,0-0.2,0-0.3c0-0.8-0.1-1.6-0.1-2.5c-0.6-19.8-1.9-66.1-45.3-80.9c-0.3-0.1-0.7-0.2-1-0.3--}}
                        {{--c-45.1-11.5-82.6-37.5-83-37.8c-6.1-4.3-14.5-2.8-18.8,3.3c-4.3,6.1-2.8,14.5,3.3,18.8c1.7,1.2,41.5,28.9,91.3,41.7--}}
                        {{--c23.3,8.3,25.9,33.2,26.6,56c0,0.9,0,1.7,0.1,2.5c0.1,9-0.5,22.9-2.1,30.9c-16.2,9.2-79.7,41-176.3,41--}}
                        {{--c-96.2,0-160.1-31.9-176.4-41.1c-1.6-8-2.3-21.9-2.1-30.9c0-0.8,0.1-1.6,0.1-2.5c0.7-22.8,3.3-47.7,26.6-56--}}
                        {{--c49.8-12.8,89.6-40.6,91.3-41.7c6.1-4.3,7.6-12.7,3.3-18.8c-4.3-6.1-12.7-7.6-18.8-3.3c-0.4,0.3-37.7,26.3-83,37.8--}}
                        {{--c-0.4,0.1-0.7,0.2-1,0.3c-43.4,14.9-44.7,61.2-45.3,80.9c0,0.9,0,1.7-0.1,2.5c0,0.1,0,0.2,0,0.3c-0.1,5.2-0.2,31.9,5.1,45.3--}}
                        {{--c1,2.6,2.8,4.8,5.2,6.3c3,2,74.9,47.8,195.2,47.8s192.2-45.9,195.2-47.8c2.3-1.5,4.2-3.7,5.2-6.3--}}
                        {{--C447,415.5,446.9,388.8,446.8,383.6z" fill="#007bff"/>--}}
                        {{--</svg>--}}
                        <img src="{{ $data['picture']}}">

                    </div>
                    <div>
                        <h5 class="h5 mb-1">
                            {{ $args['first_name'].' '.$args['last_name'] }}
                        </h5>
                        @isset($data['basic']['headline'])
                        <p class="mb-1 d-flex align-item-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="d-inline-block mr-2" width="25px"
                                 height="25px" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 512.019 512.019"
                                 style="enable-background:new 0 0 512.019 512.019;" xml:space="preserve">
                                                    <path d="M480.009,106.676h-448c-17.643,0-32,14.357-32,32v298.667c0,17.643,14.357,32,32,32h448c17.643,0,32-14.357,32-32V138.676    C512.009,121.033,497.652,106.676,480.009,106.676z M490.676,437.343c0,5.888-4.779,10.667-10.667,10.667h-448    c-5.888,0-10.667-4.779-10.667-10.667V138.676c0-5.888,4.779-10.667,10.667-10.667h448c5.888,0,10.667,4.779,10.667,10.667    V437.343z"
                                                          fill="#007bff"/>
                                <path d="M309.343,42.676H202.676c-17.643,0-32,14.357-32,32v42.667c0,5.888,4.779,10.667,10.667,10.667h149.333    c5.888,0,10.667-4.779,10.667-10.667V74.676C341.343,57.033,326.985,42.676,309.343,42.676z M320.009,106.676h-128v-32    c0-5.888,4.779-10.667,10.667-10.667h106.667c5.888,0,10.667,4.779,10.667,10.667V106.676z"
                                      fill="#007bff"/>
                                <path d="M511.668,242.655c-1.493-5.717-7.403-9.088-13.013-7.637l-169.963,44.331c-47.552,12.416-97.835,12.416-145.387,0    L13.364,235.017c-5.611-1.451-11.541,1.92-13.013,7.637c-1.493,5.696,1.92,11.52,7.637,13.013l169.941,44.331    c25.557,6.656,51.819,9.984,78.08,9.984s52.544-3.328,78.08-9.984l169.941-44.331    C509.748,254.175,513.161,248.351,511.668,242.655z"
                                      fill="#007bff"/>
                                <path d="M256.009,192.009c-23.531,0-42.667,19.136-42.667,42.667c0,23.531,19.136,42.667,42.667,42.667    s42.667-19.136,42.667-42.667C298.676,211.145,279.54,192.009,256.009,192.009z M256.009,256.009    c-11.755,0-21.333-9.579-21.333-21.333c0-11.755,9.579-21.333,21.333-21.333c11.755,0,21.333,9.579,21.333,21.333    C277.343,246.431,267.764,256.009,256.009,256.009z"
                                      fill="#007bff"/>
                                            </svg>
                            {{ $data['basic']['headline'] or "" }}
                        </p>
                        @endif
                        <p class="mb-1 d-flex align-item-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="d-inline-block mr-2" width="25px"
                                 height="25px" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 512.019 512.019"
                                 style="enable-background:new 0 0 512.019 512.019;" xml:space="preserve">
        <path d="M480.009,106.676h-448c-17.643,0-32,14.357-32,32v298.667c0,17.643,14.357,32,32,32h448c17.643,0,32-14.357,32-32V138.676    C512.009,121.033,497.652,106.676,480.009,106.676z M490.676,437.343c0,5.888-4.779,10.667-10.667,10.667h-448    c-5.888,0-10.667-4.779-10.667-10.667V138.676c0-5.888,4.779-10.667,10.667-10.667h448c5.888,0,10.667,4.779,10.667,10.667    V437.343z"
              fill="#007bff"/>
                                <path d="M309.343,42.676H202.676c-17.643,0-32,14.357-32,32v42.667c0,5.888,4.779,10.667,10.667,10.667h149.333    c5.888,0,10.667-4.779,10.667-10.667V74.676C341.343,57.033,326.985,42.676,309.343,42.676z M320.009,106.676h-128v-32    c0-5.888,4.779-10.667,10.667-10.667h106.667c5.888,0,10.667,4.779,10.667,10.667V106.676z"
                                      fill="#007bff"/>
                                <path d="M511.668,242.655c-1.493-5.717-7.403-9.088-13.013-7.637l-169.963,44.331c-47.552,12.416-97.835,12.416-145.387,0    L13.364,235.017c-5.611-1.451-11.541,1.92-13.013,7.637c-1.493,5.696,1.92,11.52,7.637,13.013l169.941,44.331    c25.557,6.656,51.819,9.984,78.08,9.984s52.544-3.328,78.08-9.984l169.941-44.331    C509.748,254.175,513.161,248.351,511.668,242.655z"
                                      fill="#007bff"/>
                                <path d="M256.009,192.009c-23.531,0-42.667,19.136-42.667,42.667c0,23.531,19.136,42.667,42.667,42.667    s42.667-19.136,42.667-42.667C298.676,211.145,279.54,192.009,256.009,192.009z M256.009,256.009    c-11.755,0-21.333-9.579-21.333-21.333c0-11.755,9.579-21.333,21.333-21.333c11.755,0,21.333,9.579,21.333,21.333    C277.343,246.431,267.764,256.009,256.009,256.009z"
                                      fill="#007bff"/>
</svg>
                            {{ $data['location'] }}
                        </p>
                        @if($data['phone'])
                            <p class="mb-1 d-flex align-item-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="d-inline-block mr-2" width="25px"
                                     height="25px" version="1.1" id="Layer_1" x="0px" y="0px"
                                     viewBox="0 0 512.019 512.019"
                                     style="enable-background:new 0 0 512.019 512.019;" xml:space="preserve">
        <path d="M480.009,106.676h-448c-17.643,0-32,14.357-32,32v298.667c0,17.643,14.357,32,32,32h448c17.643,0,32-14.357,32-32V138.676    C512.009,121.033,497.652,106.676,480.009,106.676z M490.676,437.343c0,5.888-4.779,10.667-10.667,10.667h-448    c-5.888,0-10.667-4.779-10.667-10.667V138.676c0-5.888,4.779-10.667,10.667-10.667h448c5.888,0,10.667,4.779,10.667,10.667    V437.343z"
              fill="#007bff"/>
                                    <path d="M309.343,42.676H202.676c-17.643,0-32,14.357-32,32v42.667c0,5.888,4.779,10.667,10.667,10.667h149.333    c5.888,0,10.667-4.779,10.667-10.667V74.676C341.343,57.033,326.985,42.676,309.343,42.676z M320.009,106.676h-128v-32    c0-5.888,4.779-10.667,10.667-10.667h106.667c5.888,0,10.667,4.779,10.667,10.667V106.676z"
                                          fill="#007bff"/>
                                    <path d="M511.668,242.655c-1.493-5.717-7.403-9.088-13.013-7.637l-169.963,44.331c-47.552,12.416-97.835,12.416-145.387,0    L13.364,235.017c-5.611-1.451-11.541,1.92-13.013,7.637c-1.493,5.696,1.92,11.52,7.637,13.013l169.941,44.331    c25.557,6.656,51.819,9.984,78.08,9.984s52.544-3.328,78.08-9.984l169.941-44.331    C509.748,254.175,513.161,248.351,511.668,242.655z"
                                          fill="#007bff"/>
                                    <path d="M256.009,192.009c-23.531,0-42.667,19.136-42.667,42.667c0,23.531,19.136,42.667,42.667,42.667    s42.667-19.136,42.667-42.667C298.676,211.145,279.54,192.009,256.009,192.009z M256.009,256.009    c-11.755,0-21.333-9.579-21.333-21.333c0-11.755,9.579-21.333,21.333-21.333c11.755,0,21.333,9.579,21.333,21.333    C277.343,246.431,267.764,256.009,256.009,256.009z"
                                          fill="#007bff"/>
</svg>
                                {{ $data['phone'] }}
                            </p>
                        @endif
                        <p class="mb-1 d-flex align-item-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="d-inline-block mr-2" width="25px"
                                 height="25px" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 512.019 512.019"
                                 style="enable-background:new 0 0 512.019 512.019;" xml:space="preserve">
        <path d="M480.009,106.676h-448c-17.643,0-32,14.357-32,32v298.667c0,17.643,14.357,32,32,32h448c17.643,0,32-14.357,32-32V138.676    C512.009,121.033,497.652,106.676,480.009,106.676z M490.676,437.343c0,5.888-4.779,10.667-10.667,10.667h-448    c-5.888,0-10.667-4.779-10.667-10.667V138.676c0-5.888,4.779-10.667,10.667-10.667h448c5.888,0,10.667,4.779,10.667,10.667    V437.343z"
              fill="#007bff"/>
                                <path d="M309.343,42.676H202.676c-17.643,0-32,14.357-32,32v42.667c0,5.888,4.779,10.667,10.667,10.667h149.333    c5.888,0,10.667-4.779,10.667-10.667V74.676C341.343,57.033,326.985,42.676,309.343,42.676z M320.009,106.676h-128v-32    c0-5.888,4.779-10.667,10.667-10.667h106.667c5.888,0,10.667,4.779,10.667,10.667V106.676z"
                                      fill="#007bff"/>
                                <path d="M511.668,242.655c-1.493-5.717-7.403-9.088-13.013-7.637l-169.963,44.331c-47.552,12.416-97.835,12.416-145.387,0    L13.364,235.017c-5.611-1.451-11.541,1.92-13.013,7.637c-1.493,5.696,1.92,11.52,7.637,13.013l169.941,44.331    c25.557,6.656,51.819,9.984,78.08,9.984s52.544-3.328,78.08-9.984l169.941-44.331    C509.748,254.175,513.161,248.351,511.668,242.655z"
                                      fill="#007bff"/>
                                <path d="M256.009,192.009c-23.531,0-42.667,19.136-42.667,42.667c0,23.531,19.136,42.667,42.667,42.667    s42.667-19.136,42.667-42.667C298.676,211.145,279.54,192.009,256.009,192.009z M256.009,256.009    c-11.755,0-21.333-9.579-21.333-21.333c0-11.755,9.579-21.333,21.333-21.333c11.755,0,21.333,9.579,21.333,21.333    C277.343,246.431,267.764,256.009,256.009,256.009z"
                                      fill="#007bff"/>
</svg>
                            {{ $args['email'] }}
                        </p>
                        @if($args['basic']['website'])
                            <p class="mb-1 d-flex align-item-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="d-inline-block mr-2" width="25px"
                                     height="25px" version="1.1" id="Layer_1" x="0px" y="0px"
                                     viewBox="0 0 512.019 512.019"
                                     style="enable-background:new 0 0 512.019 512.019;" xml:space="preserve">
        <path d="M480.009,106.676h-448c-17.643,0-32,14.357-32,32v298.667c0,17.643,14.357,32,32,32h448c17.643,0,32-14.357,32-32V138.676    C512.009,121.033,497.652,106.676,480.009,106.676z M490.676,437.343c0,5.888-4.779,10.667-10.667,10.667h-448    c-5.888,0-10.667-4.779-10.667-10.667V138.676c0-5.888,4.779-10.667,10.667-10.667h448c5.888,0,10.667,4.779,10.667,10.667    V437.343z"
              fill="#007bff"/>
                                    <path d="M309.343,42.676H202.676c-17.643,0-32,14.357-32,32v42.667c0,5.888,4.779,10.667,10.667,10.667h149.333    c5.888,0,10.667-4.779,10.667-10.667V74.676C341.343,57.033,326.985,42.676,309.343,42.676z M320.009,106.676h-128v-32    c0-5.888,4.779-10.667,10.667-10.667h106.667c5.888,0,10.667,4.779,10.667,10.667V106.676z"
                                          fill="#007bff"/>
                                    <path d="M511.668,242.655c-1.493-5.717-7.403-9.088-13.013-7.637l-169.963,44.331c-47.552,12.416-97.835,12.416-145.387,0    L13.364,235.017c-5.611-1.451-11.541,1.92-13.013,7.637c-1.493,5.696,1.92,11.52,7.637,13.013l169.941,44.331    c25.557,6.656,51.819,9.984,78.08,9.984s52.544-3.328,78.08-9.984l169.941-44.331    C509.748,254.175,513.161,248.351,511.668,242.655z"
                                          fill="#007bff"/>
                                    <path d="M256.009,192.009c-23.531,0-42.667,19.136-42.667,42.667c0,23.531,19.136,42.667,42.667,42.667    s42.667-19.136,42.667-42.667C298.676,211.145,279.54,192.009,256.009,192.009z M256.009,256.009    c-11.755,0-21.333-9.579-21.333-21.333c0-11.755,9.579-21.333,21.333-21.333c11.755,0,21.333,9.579,21.333,21.333    C277.343,246.431,267.764,256.009,256.009,256.009z"
                                          fill="#007bff"/>
</svg>
                                {{ $args['basic']['website'] or "" }}
                            </p>
                        @endif
                    </div>
                </div>
            </div>

            <div class="col-md-1">
                <p><a href="/user/resume/create?tab=basic" class="text-muted" style="font-size: 14px;">Edit <img
                                src="{{ asset('img/edit.png') }}">
                    </a></p>
            </div>


        </div>
    </div>
    <div class="card-body p-0">

        <div class="collapse collapse-notes py-4 border border-top-0 border-left-0 border-right-0" id="collapse-notes">
            <div class="row justify-content-center">
                <div class="col-11">
                    <div class="row">
                        <div class="col-12">
                            <div class="mb-4 bg-white">
                                <button type="button"
                                        class="btn btn-lg btn-outline-primary w-100 p-4"
                                        data-toggle="modal"
                                        data-target="#edit-notes-modal">
                                                                    <span class="d-block h3 mb-0">
                                                                        <svg id="Layer_1" width="45px" height="45px"
                                                                             style="enable-background:new 0 0 512 512;"
                                                                             version="1.1" viewBox="0 0 512 512"
                                                                             xml:space="preserve"
                                                                             xmlns="http://www.w3.org/2000/svg"><g><g><g><path
                                                                                                d="M256,48C141.1,48,48,141.1,48,256s93.1,208,208,208c114.9,0,208-93.1,208-208S370.9,48,256,48z M256,446.7     c-105.1,0-190.7-85.5-190.7-190.7S150.9,65.3,256,65.3S446.7,150.9,446.7,256S361.1,446.7,256,446.7z"
                                                                                                fill="#007bff"/></g></g><g><polygon
                                                                                            points="264.1,128 247.3,128 247.3,247.9 128,247.9 128,264.7 247.3,264.7 247.3,384 264.1,384 264.1,264.7 384,264.7     384,247.9 264.1,247.9  "
                                                                                            fill="#007bff"/></g></g></svg>
                                                                    </span>
                                    Add Note
                                </button>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="border bg-light rounded p-2 mb-4">
                                <p class="mb-0" style="opacity: 0.8;">No notes added yet!</p>
                            </div>
                        </div>
                    </div>
                    <div class="card">
                        <div class="card-header text-left d-flex justify-content-between align-items-center">
                            <h5 class="h5 mb-0">Notes history</h5>
                            <div class="d-inline-block bg-white">
                                <button type="button"
                                        class="btn btn-outline-primary ">
                                    Sort by
                                </button>
                            </div>
                        </div>
                        <div class="card-body px-0">
                            <div class="row justify-content-center">
                                <div class="col-11">
                                    <div class="form-group mb-5">
                                        <input type="text" class="form-control bg-light"
                                               placeholder="Search by name, note title or date">
                                    </div>
                                    <div class="card text-left mb-3">
                                        <div class="card-header d-flex justify-content-between">
                                            <h6 class="h6 mb-0">Manager full name</h6>
                                            <p class="mb-0 font-weight-bold">20/09/2017</p>
                                        </div>
                                        <div class="card-body">
                                            <h5 class="mb-2">Note title</h5>
                                            <p class="mb-0">Lorem ipsum dolor sit amet, consectetur adipisicing elit.
                                                Aspernatur atque, blanditiis dolorem dolorum, ea, eveniet inventore nemo
                                                nisi nostrum praesentium repudiandae veritatis! Ab aspernatur
                                                consectetur, distinctio dolores facere quae sint.</p>
                                        </div>
                                    </div>
                                    <div class="card text-left">
                                        <div class="card-header d-flex justify-content-between">
                                            <h6 class="h6 mb-0">Manager full name</h6>
                                            <p class="mb-0 font-weight-bold">20/09/2017</p>
                                        </div>
                                        <div class="card-body">
                                            <h5 class="mb-2">Note title</h5>
                                            <p class="mb-0">Lorem ipsum dolor sit amet, consectetur adipisicing elit.
                                                Aspernatur atque, blanditiis dolorem dolorum, ea, eveniet inventore nemo
                                                nisi nostrum praesentium repudiandae veritatis! Ab aspernatur
                                                consectetur, distinctio dolores facere quae sint.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="modal fade" id="edit-notes-modal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Add note</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <div class="conteiner">
                            <div class="row justify-content-center">
                                <div class="col-11">
                                    <div class="row text-left">
                                        <div class="col-12">
                                            <div class="form-group mb-3">
                                                <label>Note title</label>
                                                <input type="text" class="form-control bg-light"
                                                       placeholder="Enter note title">
                                            </div>
                                            <div class="form-group mb-0">
                                                <label>Note description</label>
                                                <textarea class="form-control bg-light" rows="6"></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer justify-content-center bg-light">
                        <div class="bg-white">
                            <button type="button" class="btn btn-outline-primary">
                                Save
                            </button>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="border border-top-0 border-left-0 border-right-0">
            <div class="row justify-content-center">
                <div class="col-11">
                    <div class="row">
                        <div class="col-3">
                            <div class="text-center py-3">
                                <h6 class="h6 mb-0">Open to new job</h6>
                                <p class="mb-0">{{ ucfirst($args['preference']['new_job']) }}</p>
                            </div>
                        </div>
                        <div class="col-3">
                            <div class="text-center py-3">
                                <h6 class="h6 mb-0">Open to opportunities</h6>
                                <p class="mb-0">{{ ucfirst($args['preference']['new_opportunities']) }}</p>
                            </div>
                        </div>
                        <div class="col-3">
                            <div class="text-center py-3">
                                <h6 class="h6 mb-0">Open to opportunities</h6>
                                <p class="mb-0">Yes</p>
                            </div>
                        </div>
                        <div class="col-3">
                            <div class="text-center py-3">
                                <h6 class="h6 mb-0">Open to opportunities</h6>
                                <p class="mb-0">Yes</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        @if($args['basic']['about'])
            <div class="border border-top-0 border-left-0 border-right-0 text-left">
                <div class="row justify-content-center">
                    <div class="col-11">
                        <div class="py-3">
                            <h5 class="h5 mb-2">About "{{ $args['first_name'] . ' ' . $args['last_name'] }}"</h5>
                            <p class="mb-0 text-muted">{{ $args['basic']['about'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
        @endif
        <div class="border border-top-0 border-left-0 border-right-0 text-left">
            <div class="row justify-content-center">
                <div class="col-11">
                    <div class="table py-3">
                        <p style="text-align: right"><a href="/user/resume/create?tab=availabilities" class="text-muted"
                                                        style="font-size: 14px;">Edit <img
                                        src="{{ asset('img/edit.png') }}">
                            </a></p>
                        <table class="w-100" style="table-layout: fixed">
                            <thead>
                            <tr class="text-center">
                                <th colspan="2" class="table-heading border-0 text-center"></th>
                                <th class="table-heading border-0 text-center">
                                    <svg version="1.1" id="Layer_1" width="20px" height="20px"
                                         xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                         viewBox="0 0 91 91" enable-background="new 0 0 91 91" xml:space="preserve">
    <path d="M45.5,32.4c2.2,0,4-1.8,4-4v-8.1c0-2.2-1.8-4-4-4s-4,1.8-4,4v8.1C41.5,30.7,43.3,32.4,45.5,32.4z"
          fill="#007bff"/>
                                        <path d="M69,42c1,0,2-0.4,2.8-1.2l5.8-5.8c1.6-1.6,1.6-4.1,0-5.7c-1.6-1.6-4.1-1.6-5.7,0l-5.8,5.8c-1.6,1.6-1.6,4.1,0,5.7
        C67,41.6,68,42,69,42z" fill="#007bff"/>
                                        <path d="M19.2,40.8C19.9,41.6,21,42,22,42c1,0,2-0.4,2.8-1.2c1.6-1.6,1.6-4.1,0-5.7l-5.8-5.8c-1.6-1.6-4.1-1.6-5.7,0
        c-1.6,1.6-1.6,4.1,0,5.7L19.2,40.8z" fill="#007bff"/>
                                        <path d="M86.9,66.7H4.1c-2.2,0-4,1.8-4,4s1.8,4,4,4h82.8c2.2,0,4-1.8,4-4S89.1,66.7,86.9,66.7z"
                                              fill="#007bff"/>
                                        <path d="M27.1,60.8c2.1,0.6,4.3-0.7,4.9-2.9c1.6-6.2,7.2-10.5,13.6-10.5s12,4.3,13.6,10.5c0.5,1.8,2.1,3,3.9,3c0.3,0,0.7,0,1-0.1
        c2.1-0.6,3.4-2.7,2.9-4.9c-2.5-9.7-11.3-16.5-21.3-16.5c-10,0-18.8,6.8-21.3,16.5C23.6,58.1,24.9,60.3,27.1,60.8z"
                                              fill="#007bff"/>
</svg>
                                </th>
                                <th class="table-heading border-0 text-center">
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" width="20px"
                                         height="20px" x="0px" y="0px"
                                         viewBox="0 0 91 91" enable-background="new 0 0 91 91" xml:space="preserve">
    <path d="M45.5,23.5c-12.1,0-22,9.9-22,22c0,12.1,9.9,22,22,22c12.1,0,22-9.9,22-22C67.5,33.4,57.6,23.5,45.5,23.5z M45.5,59.5
        c-7.7,0-14-6.3-14-14c0-7.7,6.3-14,14-14c7.7,0,14,6.3,14,14C59.5,53.2,53.2,59.5,45.5,59.5z" fill="#007bff"/>
                                        <path d="M45.5,16.2c2.2,0,4-1.8,4-4V4.1c0-2.2-1.8-4-4-4c-2.2,0-4,1.8-4,4v8.1C41.5,14.5,43.3,16.2,45.5,16.2z"
                                              fill="#007bff"/>
                                        <path d="M86.9,41.5h-8.1c-2.2,0-4,1.8-4,4c0,2.2,1.8,4,4,4h8.1c2.2,0,4-1.8,4-4C90.9,43.3,89.1,41.5,86.9,41.5z"
                                              fill="#007bff"/>
                                        <path d="M45.5,74.8c-2.2,0-4,1.8-4,4v8.1c0,2.2,1.8,4,4,4c2.2,0,4-1.8,4-4v-8.1C49.5,76.5,47.7,74.8,45.5,74.8z"
                                              fill="#007bff"/>
                                        <path d="M16.2,45.5c0-2.2-1.8-4-4-4H4.1c-2.2,0-4,1.8-4,4c0,2.2,1.8,4,4,4h8.1C14.5,49.5,16.2,47.7,16.2,45.5z"
                                              fill="#007bff"/>
                                        <path d="M69,26c1,0,2-0.4,2.8-1.2l5.8-5.8c1.6-1.6,1.6-4.1,0-5.7c-1.6-1.6-4.1-1.6-5.7,0l-5.8,5.8c-1.6,1.6-1.6,4.1,0,5.7
        C67,25.6,68,26,69,26z" fill="#007bff"/>
                                        <path d="M71.8,66.2c-1.6-1.6-4.1-1.6-5.7,0c-1.6,1.6-1.6,4.1,0,5.7l5.8,5.8c0.8,0.8,1.8,1.2,2.8,1.2c1,0,2-0.4,2.8-1.2
        c1.6-1.6,1.6-4.1,0-5.7L71.8,66.2z" fill="#007bff"/>
                                        <path d="M19.2,66.2l-5.8,5.8c-1.6,1.6-1.6,4.1,0,5.7c0.8,0.8,1.8,1.2,2.8,1.2c1,0,2-0.4,2.8-1.2l5.8-5.8c1.6-1.6,1.6-4.1,0-5.7
        C23.3,64.6,20.7,64.6,19.2,66.2z" fill="#007bff"/>
                                        <path d="M19.2,24.8C19.9,25.6,21,26,22,26c1,0,2-0.4,2.8-1.2c1.6-1.6,1.6-4.1,0-5.7l-5.8-5.8c-1.6-1.6-4.1-1.6-5.7,0
        c-1.6,1.6-1.6,4.1,0,5.7L19.2,24.8z" fill="#007bff"/>
</svg>
                                </th>
                                <th class="table-heading border-0 text-center">
                                    <svg version="1.1" id="Layer_1" width="20px" height="20px"
                                         xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                         viewBox="0 0 91 91" enable-background="new 0 0 91 91" xml:space="preserve">
    <path d="M45.5,26.3c2.2,0,4-1.8,4-4v-8.1c0-2.2-1.8-4-4-4s-4,1.8-4,4v8.1C41.5,24.5,43.3,26.3,45.5,26.3z"
          fill="#007bff"/>
                                        <path d="M74.8,55.6c0,2.2,1.8,4,4,4h8.1c2.2,0,4-1.8,4-4s-1.8-4-4-4h-8.1C76.5,51.6,74.8,53.4,74.8,55.6z"
                                              fill="#007bff"/>
                                        <path d="M4.1,59.6h8.1c2.2,0,4-1.8,4-4s-1.8-4-4-4H4.1c-2.2,0-4,1.8-4,4S1.9,59.6,4.1,59.6z"
                                              fill="#007bff"/>
                                        <path d="M69,36.1c1,0,2-0.4,2.8-1.2l5.8-5.8c1.6-1.6,1.6-4.1,0-5.7c-1.6-1.6-4.1-1.6-5.7,0l-5.8,5.8c-1.6,1.6-1.6,4.1,0,5.7
        C67,35.7,68,36.1,69,36.1z" fill="#007bff"/>
                                        <path d="M19.2,34.9c0.8,0.8,1.8,1.2,2.8,1.2c1,0,2-0.4,2.8-1.2c1.6-1.6,1.6-4.1,0-5.7l-5.8-5.8c-1.6-1.6-4.1-1.6-5.7,0
        c-1.6,1.6-1.6,4.1,0,5.7L19.2,34.9z" fill="#007bff"/>
                                        <path d="M25.5,64.7c0.9,2,3.3,2.9,5.3,2c2-0.9,2.9-3.3,2-5.3c-0.8-1.8-1.3-3.8-1.3-5.8c0-7.7,6.3-14,14-14s14,6.3,14,14
        c0,2-0.4,4-1.3,5.8c-0.9,2,0,4.4,2,5.3c0.5,0.2,1.1,0.4,1.7,0.4c1.5,0,3-0.9,3.6-2.3c1.3-2.9,2-6,2-9.1c0-12.1-9.9-22-22-22
        c-12.1,0-22,9.9-22,22C23.5,58.8,24.2,61.8,25.5,64.7z" fill="#007bff"/>
                                        <path d="M86.9,72.8H4.1c-2.2,0-4,1.8-4,4s1.8,4,4,4h82.8c2.2,0,4-1.8,4-4S89.1,72.8,86.9,72.8z"
                                              fill="#007bff"/>
</svg>
                                </th>
                                <th class="table-heading border-0 text-center">
                                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" width="20px"
                                         height="20px" x="0px" y="0px"
                                         viewBox="0 0 91 91" enable-background="new 0 0 91 91" xml:space="preserve">
<path d="M47.2,78.1c-11.6,0-22.5-6.3-28.3-16.3c-9-15.6-3.6-35.6,11.9-44.6c5.3-3.1,11.5-4.6,17.6-4.3c1.5,0.1,2.8,0.9,3.4,2.2
    c0.7,1.3,0.5,2.9-0.3,4.1c-5,7.2-5.4,16.8-1,24.3c4.1,7,11.6,11.4,19.8,11.4c0.6,0,1.2,0,1.8-0.1c1.5-0.1,2.9,0.6,3.7,1.8
    c0.8,1.2,0.9,2.8,0.2,4.1c-2.9,5.5-7.2,10-12.6,13.1C58.5,76.6,52.9,78.1,47.2,78.1z M41.4,21.5c-2.3,0.5-4.5,1.4-6.6,2.6
    C23.1,31,19,46.1,25.8,57.8c4.4,7.6,12.6,12.3,21.4,12.3c4.3,0,8.5-1.1,12.3-3.3c2.1-1.2,4-2.7,5.6-4.4c-8.9-1.6-16.8-7-21.4-14.9
    C39.1,39.5,38.4,30,41.4,21.5z" fill="#007bff"/>
</svg>
                                </th>
                            </tr>
                            </thead>
                            <tbody class="text-left">
                            <?php
                            $days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
                            ?>
                            @for($d = 1; $d <= 7; ++$d)
                                <tr class="text-center">
                                    <td colspan="2" class="text-left">{{ $days[$d - 1] }}</td>
                                    @for($i = 1; $i <= 4; ++$i)
                                        <?php
                                        $checkbox = false;
                                        ?>
                                        @isset($args['availability']['time_'.$i])
                                            <?php
                                            $checkbox = strpos($args['availability']['time_' . $i], (string)$d);
                                            ?>
                                        @endisset
                                        <td class="align-middle">
                                            <div class="d-flex align-items-center justify-content-center">
                                                <label class="custom-control custom-checkbox m-0 pl-3">
                                                    <input type="checkbox"
                                                           class="custom-control-input"
                                                           @if($checkbox !== false) checked="checked"
                                                           @endif onclick="return false;">
                                                    <span class="custom-control-indicator"></span>
                                                </label>
                                            </div>
                                        </td>
                                    @endfor
                                </tr>
                            @endfor

                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
        @if(count($args['reference']) > 0)
            <div class="text-left">
                <div class="row justify-content-center">
                    <div class="col-11">
                        <div class="py-3">
                            <p class="mb-0 text-right"><a href="/user/resume/create?tab=references" class="text-muted"
                                                          style="font-size: 14px;">Edit <img
                                            src="{{ asset('img/edit.png') }}"> </a></p>
                            <h5 class="h5 mb-2">About "{{ $args['first_name'] . ' ' . $args['last_name'] }}"</h5>
                            <div class="row">
                                @foreach($args['reference'] as $reference)
                                    <div class="col-4">
                                        <div class="card text-left mb-0">
                                            <div class="card-header">
                                                <h6 class="h6 mb-0">{{ $reference['full_name'] }}</h6>
                                            </div>
                                            <div class="card-body">
                                                <p class="mb-0 font-weight-bold">{{ $reference['company'] }}</p>
                                                {{--<p class="mb-0">Message from referrer</p>--}}
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    </div>
    @endif
</div>
@if(count($args['experience']) > 0)
    <div class="card mb-5">
        <div class="card-header pb-4 pt-0">
            <p class="mb-0 text-right"><a href="/user/resume/create?tab=experience" class="text-muted"
                                          style="font-size: 14px;">Edit <img
                            src="{{ asset('img/edit.png') }}"> </a></p>
            <div>

                <svg xmlns="http://www.w3.org/2000/svg" class="d-block mx-auto mb-1" width="50px" height="50px"
                     version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 512.019 512.019"
                     style="enable-background:new 0 0 512.019 512.019;" xml:space="preserve">
        <path d="M480.009,106.676h-448c-17.643,0-32,14.357-32,32v298.667c0,17.643,14.357,32,32,32h448c17.643,0,32-14.357,32-32V138.676    C512.009,121.033,497.652,106.676,480.009,106.676z M490.676,437.343c0,5.888-4.779,10.667-10.667,10.667h-448    c-5.888,0-10.667-4.779-10.667-10.667V138.676c0-5.888,4.779-10.667,10.667-10.667h448c5.888,0,10.667,4.779,10.667,10.667    V437.343z"
              fill="#007bff"/>
                    <path d="M309.343,42.676H202.676c-17.643,0-32,14.357-32,32v42.667c0,5.888,4.779,10.667,10.667,10.667h149.333    c5.888,0,10.667-4.779,10.667-10.667V74.676C341.343,57.033,326.985,42.676,309.343,42.676z M320.009,106.676h-128v-32    c0-5.888,4.779-10.667,10.667-10.667h106.667c5.888,0,10.667,4.779,10.667,10.667V106.676z"
                          fill="#007bff"/>
                    <path d="M511.668,242.655c-1.493-5.717-7.403-9.088-13.013-7.637l-169.963,44.331c-47.552,12.416-97.835,12.416-145.387,0    L13.364,235.017c-5.611-1.451-11.541,1.92-13.013,7.637c-1.493,5.696,1.92,11.52,7.637,13.013l169.941,44.331    c25.557,6.656,51.819,9.984,78.08,9.984s52.544-3.328,78.08-9.984l169.941-44.331    C509.748,254.175,513.161,248.351,511.668,242.655z"
                          fill="#007bff"/>
                    <path d="M256.009,192.009c-23.531,0-42.667,19.136-42.667,42.667c0,23.531,19.136,42.667,42.667,42.667    s42.667-19.136,42.667-42.667C298.676,211.145,279.54,192.009,256.009,192.009z M256.009,256.009    c-11.755,0-21.333-9.579-21.333-21.333c0-11.755,9.579-21.333,21.333-21.333c11.755,0,21.333,9.579,21.333,21.333    C277.343,246.431,267.764,256.009,256.009,256.009z"
                          fill="#007bff"/>
</svg>
                <h4 class="h4 mb-0">Experience</h4>
            </div>
        </div>
        <div class="card-body px-0 py-4">
            <div class="row justify-content-center">
                <div class="col-11">
                    @foreach($args['experience'] as $experience)
                        <?php
                        $location = $experience['city'];
                        if ($experience['region'] != "") {
                            $location .= "," . $experience['region'];
                        }
                        if ($experience['country'] != "") {
                            $location .= "," . $experience['country'];
                        }
                        ?>
                        <div class="card text-left mb-3">
                            <div class="card-header">
                                <h6 class="h6 mb-0">{{ date('F',strtotime($experience['date_from'])) }} {{ date('Y',strtotime($experience['date_from'])) }}
                                    -
                                    {{ date('F',strtotime($experience['date_to'])) }} {{ date('Y',strtotime($experience['date_to'])) }}</h6>
                            </div>
                            <div class="card-body p-0">
                                <div class="row no-gutters">
                                    <div class="col-2 text-center">
                                        <div class="px-2 py-3 h-100 border border-left-0 border-top-0 border-bottom-0">
                                            <div class="rounded p-1 bg-white d-inline-block"
                                                 style="box-shadow: 0 4px 10px 0 rgba(0, 0, 0, 0.14), 0 1px 2px 0 rgba(0, 0, 0, 0.12), 0 3px 1px -2px rgba(0, 0, 0, 0.2);">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="60px" height="60px"
                                                     enable-background="new 0 0 800 261.953" xml:space="preserve"
                                                     viewBox="0 0 201.99998 67.488425" version="1.1" y="0px" x="0px">
                                                                                        <path fill="#f40009"
                                                                                              d="m168.12 56.832c-0.003 0-0.6185 0.54875-0.6185 0.54875-0.8795 0.78625-1.7845 1.628-2.877 1.207-0.30175-0.10975-0.514-0.51175-0.565-0.82375-0.0497-2.2505 0.96375-4.3358 1.9465-6.3662l0.252-0.531c2.827-4.7202 6.113-10.282 10.939-14.142 0.81325-0.58675 1.6958-1.0798 2.6272-0.69525 0.21425 0.20075 0.46375 0.54875 0.46375 0.896 0 0.11025-0.0977 0.40275-0.15425 0.49375-1.3478 2.1782-2.6315 4.465-3.8798 6.6602-2.4395 4.3365-4.9702 8.8185-8.134 12.753zm-26.149-11.124c-0.18475 0.12875-3.445-0.9695-4.075-3.9882-0.5345-2.599 1.2552-4.6298 2.9868-5.6535 0.74325-0.58575 1.8628-0.7865 2.684-0.495 0.76225 0.513 0.96875 1.4285 0.96875 2.415 0 0.605-0.0802 1.2265-0.151 1.7935 0 0-0.0135 0.11025-0.0157 0.1285-0.54325 2.0302-1.3745 4.0248-2.3978 5.7998zm-12.454 12.277c-0.3325-0.494-0.44275-1.0342-0.45375-1.6182-0.0638-3.5875 3.796-10.012 6.5205-13.512h0.006c1.1418 2.5 3.7272 4.107 5.288 4.7842-1.9385 4.3182-8.5868 13.566-11.36 10.346zm47.054 0.637c-0.64525 0.439-1.5518 0.1015-1.2128-0.9045 0.84425-2.5615 4.2008-7.7815 4.2008-7.7815l9.349-16.582h-6.404c-0.30425 0.5-0.95525 1.6482-0.95525 1.6482-0.29625-0.4935-1.1822-1.591-1.547-1.828-1.5645-0.98875-3.86-0.52125-5.4435 0.375-6.8848 4.0258-11.908 11.98-15.87 18.182 0 0-4.1702 6.895-6.4852 7.4078-1.8072 0.1465-1.6208-2.288-1.5425-2.8552 0.69875-4.099 2.3215-7.9958 3.9565-11.527 3.3252-2.3052 7.1202-5.324 10.569-8.6355 7.4845-7.1545 13.794-15.26 14.716-17.071 0 0-0.99825 0.2015-2.177 0.23775-5.758 8.032-17.541 19.706-21.113 21.443 1.5825-3.8062 11.807-21.974 20.508-30.664l1.3692-1.3178c2.117-2.0675 4.3108-4.1898 6.0202-4.5375 0.1805-0.0182 0.413 0 0.61375 0.4215 0.0763 1.6098-0.523 2.7992-1.2105 4.153l-0.9635 1.9763s1.3648-0.25625 2.261-0.54975c1.04-1.9568 2.1918-4.153 1.7672-6.7688-0.14175-0.84174-0.751-1.482-1.5472-1.6285-2.5955-0.494-5.4242 1.4265-7.7002 2.9822l-0.092 0.0545c-11.7 9.1665-21.58 22.377-30.218 40.417-0.626 0.475-3.0448 1.0428-3.456 0.768 0.853-1.756 1.927-4.1168 2.4645-6.6782 0.093-0.732 0.1795-1.4818 0.1795-2.2135 0-1.5748-0.378-3.0562-1.7888-4.0805-1.654-0.8235-3.6982-0.439-5.0765 0.18275-6.1745 2.5615-10.789 8.8925-14.119 14.106-1.791 3.3125-3.5768 6.8602-4.0712 10.923-0.34825 3.183 0.36075 5.2322 2.1705 6.2572 1.8612 0.95075 4.2038-0.0375 5.124-0.51275 6.2552-3.2932 10.626-9.8978 14.071-15.936 0.137-0.018 2.018-0.0725 3.4408-0.439 0.019 0 0.027 0 0.0585 0.0192-0.0815 0.27325-0.81675 2.4695-0.81675 2.4695-2.1835 6.4032-3.1932 10.758-1.1308 13.338 2.9472 3.6412 7.8265-0.1275 11.796-5.105-0.852 5.9645 2.2392 6.898 4.5955 6.441 2.7068-0.7315 5.723-3.623 7.1228-5.1045-0.4495 1.7378-0.32725 4.8485 2.135 5.1592 1.6878 0.312 2.9508-0.606 4.4122-1.393 5.2422-2.8545 11.518-12.366 13.147-15.116h-2.1362c-2.316 3.5002-5.2632 8.0895-8.9705 10.268zm-80.761-35.518h5.9998l3.3862-5.5h-6.001zm95.841-14.611c-4.5748 2.5985-9.317 4.2813-15.058 3.696-1.591 1.7935-3.14 3.6412-4.5678 5.5438 8.0572 2.0492 16.599-2.5065 21.363-6.8422 5.0395-4.373 7.6238-9.8797 7.6238-9.8797s-3.7775 4.3175-9.3612 7.4822zm-50.45-1.39c-0.954 10.941-9.4588 17.364-11.563 17.875-1.2752 0.25575-3.4512-0.311-1.5088-4.9028 2.8608-6.1848 7.7448-11.435 12.985-14.125 0.1275 0.4575 0.12425 0.7495 0.0867 1.1525zm-16.572 27.572c-0.60925-1.317-2.0558-2.1588-3.5972-2.0675-5.0058 0.45775-9.902 4.5742-12.473 10.484-1.349 3.0375-2.0852 5.5438-2.568 9.5322 1.5565-1.7925 4.7315-4.738 8.3345-6.3482 0 0 0.49025-3.8238 2.9902-7.227 0.946-1.4092 2.8392-3.6778 4.965-3.0742 1.8572 0.6225 1.2045 5.7638-1.264 10.739-1.8265 3.6598-4.6215 7.32-7.4495 9.8442-2.5002 2.1225-6.1995 4.6838-9.436 2.708-2.0385-1.207-3.0782-3.7325-2.8585-6.9892 0.974-9.2392 5.1798-17.107 11.198-26.072 6.206-8.3248 13.047-16.869 22.263-21.426 1.8798-0.95125 3.6205-1.1708 5.1252-0.58525 0 0-8.651 4.757-12.775 13.558-1.049 2.2325-2.508 5.2505-1.0388 7.922 0.76775 1.3902 2.131 1.5002 3.2285 1.427 5.0548-1.1712 8.3205-5.8915 10.989-10.209 1.547-3.275 2.922-6.5318 2.922-10.155 0-0.439-0.0228-1.0242-0.06-1.4638 2.415-1.2802 7.367 0.988 7.367 0.988 3.8698 1.2812 12.11 7.5938 14.889 8.8008 1.3558-1.5732 3.5838-3.9515 4.8382-5.141l-1.816-1.1162c-2.9075-1.7748-5.9808-3.4582-9.0058-5.123-6.8698-3.7502-12.462-2.4332-15.308-1.4085-1.1185 0.40225-2.1408 0.75-2.1408 0.75-2.112-2.3052-5.5758-2.0862-8.128-1.4637-9.1785 2.6345-17.7 9.0562-26.836 20.199-6.717 8.7275-10.884 16.266-13.104 23.712-1.7118 5.0128-2.232 12.386 1.946 16.759 3.5522 3.713 8.2625 2.9088 11.589 1.5735 7.2068-3.5495 13.684-11.198 16.912-19.998 0.77875-2.616 1.6432-6.3118 0.30225-9.1298zm-79.885-8.4888c-0.012 0.0365-1.224 2.8172-1.224 2.8172-0.17525 0.14625-0.45225 0.0733-0.76975 0l-0.544-0.092c-1.7082-0.53025-2.9968-1.6648-3.394-3.0185-0.548-2.6348 1.7028-4.7022 2.6945-5.452 0.9515-0.65925 2.4308-1.0068 3.2898-0.3295 0.52975 0.62225 0.7305 1.427 0.7305 2.3058 0.00025 1.189-0.36675 2.5245-0.783 3.769zm-2.6035 5.287s-0.055 0.16525-0.0822 0.21925c-0.007 0-2.755 4.482-2.755 4.482-1.6585 2.2142-3.7292 4.996-6.3178 6.331-0.76725 0.25625-1.8278 0.42125-2.3682-0.25525-1.1458-1.3725-0.55525-3.2752-0.0755-4.812l0.16975-0.549c1.3755-3.732 3.558-7.026 5.6822-10.082 0.0265-0.018 0.086-0.0362 0.1075-0.0362 0.005 0.0183 0.0135 0.0183 0.0172 0.055 1.333 2.561 3.8325 3.7498 5.7018 4.2995 0.0113 0 0.0233 0.0182 0.0233 0.055 0.00025 0.0545-0.0303 0.12825-0.103 0.29225zm34.376-2.9455c1.2292-1.7012 4.807-6.2572 5.6792-7.0255 2.9335-2.5808 4.106-1.4452 4.2075-0.64075-2.7378 4.9032-5.969 10.52-9.007 15.57 0.006-0.018-0.95075 1.4452-0.95075 1.4452-1.3418 2.0488-2.6928 3.8052-4.8965 5.031-0.31425 0.092-0.83225 0.12875-1.177-0.12825-0.415-0.2745-0.57875-0.732-0.532-1.208 0.132-1.573 1.7338-6.384 6.6765-13.044zm-30.465-11.124c-3.7925-2.4882-11.415 2.2688-17.52 10.428-5.583 7.3192-7.9938 15.826-5.3702 19.76 3.9028 4.6288 11.148-2.104 14.229-5.7078l0.3975-0.42125c2.0952-2.2505 3.714-4.9585 5.2808-7.5568 0 0 1.44-2.3785 1.5088-2.4885 0.89-0.164 1.962-0.42025 3.17-0.768-0.0172 0.0545-4.6218 7.794-4.152 11.636 0.14625 1.172 0.0345 5.4532 4.2145 6.752 5.571 0.93375 10.02-3.3848 13.944-7.227 0 0 0.66875-0.63975 1.047-1.005-0.0957 0.38325-0.2205 0.8945-0.2205 0.8945-1.631 5.672 0.5675 6.862 1.9428 7.3012 4.1562 1.2082 9.1215-4.83 9.1375-4.83-0.12875 1.902-0.50525 3.4755 1.4998 4.738 1.8925 0.6775 3.8078-0.35525 5.2635-1.325 5.2438-3.6958 9.4682-9.48 12.88-14.48h-2.1765c-0.0257 0-5.3708 7.747-8.8415 9.4308-0.005 0-0.637 0.32425-1.001 0.0677-0.4505-0.3845-0.27275-1.1835 0.005-1.7495 0.0175-0.037 13.401-23.249 13.401-23.249h-6.319s-0.69025 1.1168-0.7465 1.19c-0.0425-0.0365-0.14375-0.2285-0.214-0.3205-3.915-5.4155-12.781 2.9145-19.456 12.574-2.5698 3.7145-5.8415 7.8968-9.234 10.916 0 0-5.0258 4.607-7.1732 1.297-0.785-1.3908-0.5435-3.424-0.10075-4.7592 2.5002-7.1172 6.8085-13.576 11.711-17.474 1.384-1.0788 2.8882-1.3908 3.6348-0.93325 0.71125 0.4215 0.826 1.4635 0.376 2.1772-1.1418-0.0365-2.0565 0.3105-2.8002 1.079-1.465 1.537-1.974 3.0192-1.5082 4.41 2.2628 3.22 6.7755-3.1472 6.5572-7.026-0.078-1.3908-0.82-2.7078-1.9272-3.3488-1.6502-0.9875-4.1678-0.73175-5.797 0.0555-2.181 0.823-5.6335 3.6952-7.6488 5.9278-2.537 2.7998-6.888 5.9098-8.2488 5.5615 0.44575-1.1898 4.121-8.7092 0.25475-11.526zm31.762 40.709c-7.442-4.794-17.542-5.6352-33.83-1.1162-17.364 4.0805-23.154 6.6962-30.757 1.4452-2.921-2.5795-3.9683-6.7328-3.1862-12.715 1.8138-9.7525 6.7152-19.138 15.422-29.548 4.8598-5.4155 9.3675-10.063 15.32-11.929 4.562-1.1708 4.1355 2.4145 3.5835 2.9272-0.5825 0-1.561 0.0918-2.3172 0.58525-0.61375 0.476-2.2985 2.013-2.389 4.0805-0.1305 3.165 3.145 2.5062 4.5578 0.7685 1.5228-1.9582 3.7732-5.7083 2.003-9.2033-0.742-1.244-2.024-2.1037-3.6158-2.3965-5.4505-0.3115-10.527 2.2867-15.212 5.1592-10.243 7.117-18.439 16.979-23.076 27.774-2.6738 6.5132-5.0918 15.843-1.4672 23.437 2.8038 5.269 8.6248 8.0678 15.57 7.4825 4.8708-0.51175 10.76-2.086 14.722-3.0365 3.9612-0.952 24.238-7.9222 30.901 4.209 0 0 2.2155-4.3008 7.7522-4.3925 4.552-0.42 11.119 1.3178 15.901 4.83-1.5932-2.396-6.1102-5.9272-9.8785-8.361z"/>
                                                                                    </svg>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-10">
                                        <div class="p-3">
                                            <a href="#"
                                               class="h5 font-weight-bold mb-0"
                                               style="opacity: 0.8;">{{ $experience['company'] }}</a>
                                            <div class="d-flex align-items-center">
                                                <div class="mr-2">
                                            <span class="item-location-flag bfh-flag-{{ $args['country_code'] or "" }}"><i
                                                        class="glyphicon"></i> </span>
                                                </div>
                                                <p class="mb-0"
                                                   style="opacity: 0.8;">
                                                    {{ $location }}</p>
                                            </div>
                                            <p class="mb-0 font-weight-bold"
                                               style="opacity: 0.8; font-size: 15px;">
                                                {{ $experience['title'] }}</p>
                                            <p class="mb-0"
                                               style="opacity: 0.8; font-size: 15px;">
                                                {{ $experience['insudtry'] or "" }}</p>
                                        </div>
                                        @if(!empty($experience['description']))
                                            <div class="p-3 border border-right-0 border-left-0 border-bottom-0">
                                                <pre class="mb-0">{{ $experience['description'] or "" }}</pre>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                    @endforeach
                </div>
            </div>
        </div>
    </div>
@endif
@if(count($args['education']) > 0)
    <div class="card mb-5">
        <div class="card-header pb-4 pt-0">
            <p class="mb-0 text-right"><a href="/user/resume/create?tab=education" class="text-muted"
                                          style="font-size: 14px;">Edit <img
                            src="{{ asset('img/edit.png') }}"> </a></p>
            <div>
                <svg version="1.1" class="d-block mx-auto mb-1" width="50px" height="50px" id="Layer_1"
                     xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                     viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
            <path d="M360.465,443.988c-4.574-1.161-9.207,1.638-10.342,6.204l-8.533,34.133c-1.152,4.582,1.638,9.207,6.212,10.351
                c0.691,0.179,1.391,0.256,2.074,0.256c3.823,0,7.296-2.586,8.269-6.46l8.533-34.133
                C367.821,449.765,365.039,445.132,360.465,443.988z" fill="#007bff"/>
                    <path d="M256,273.066c1.109,0,2.219-0.213,3.26-0.648l93.867-38.844c4.352-1.801,6.417-6.793,4.617-11.145
                c-1.801-4.361-6.81-6.409-11.153-4.625L256,255.299L17.067,156.432v-2.833c0-4.71-3.823-8.533-8.533-8.533S0,148.889,0,153.599
                v8.533c0,3.456,2.082,6.571,5.274,7.885l247.467,102.4C253.781,272.852,254.891,273.066,256,273.066z"
                          fill="#007bff"/>
                    <path d="M357.513,360.49c-2.082-4.224-7.185-5.965-11.426-3.874c-16.205,8.013-43.273,17.331-81.553,18.603v-76.553
                c0-4.71-3.823-8.533-8.533-8.533s-8.533,3.823-8.533,8.533v76.476c-105.958-3.379-140.151-67.567-145.067-78.251v-57.958
                c0-4.71-3.823-8.533-8.533-8.533c-4.71,0-8.533,3.823-8.533,8.533v59.733c0,1.084,0.205,2.159,0.606,3.174
                c1.485,3.695,37.803,90.692,170.061,90.692c46.225,0,78.618-11.213,97.647-20.617
                C357.879,369.825,359.612,364.714,357.513,360.49z" fill="#007bff"/>
                    <path d="M384,452.266c-4.719,0-8.533,3.823-8.533,8.533v25.6c0,4.71,3.814,8.533,8.533,8.533c4.719,0,8.533-3.823,8.533-8.533
                v-25.6C392.533,456.089,388.719,452.266,384,452.266z" fill="#007bff"/>
                    <path d="M503.467,145.066c-4.719,0-8.533,3.823-8.533,8.533v2.816l-102.4,42.146v-15.42l114.193-47.258
                c3.191-1.314,5.274-4.429,5.274-7.885c0-3.456-2.082-6.562-5.265-7.885L259.26,17.714c-2.091-0.862-4.429-0.862-6.519,0
                L5.274,120.114C2.082,121.437,0,124.543,0,127.999c0,3.456,2.082,6.571,5.274,7.885l247.467,102.4
                c1.041,0.435,2.15,0.649,3.26,0.649s2.219-0.213,3.26-0.649l89.481-37.026c4.361-1.792,6.426-6.784,4.625-11.145
                c-1.801-4.352-6.793-6.409-11.145-4.617L256,221.166L30.848,127.999L256,34.832l225.152,93.167l-97.971,40.542l-93.841-35.038
                c0.495-1.775,0.794-3.601,0.794-5.504c0-14.353-14.993-25.6-34.133-25.6s-34.133,11.247-34.133,25.6
                c0,14.353,14.993,25.6,34.133,25.6c8.542,0,16.162-2.33,22.084-6.084l97.382,36.361v27.341v0.009v182.878
                c-9.907,3.541-17.067,12.919-17.067,24.03c0,14.114,11.486,25.6,25.6,25.6s25.6-11.486,25.6-25.6
                c0-11.11-7.159-20.488-17.067-24.03v-48.435c24.499-18.492,33.28-43.119,33.664-44.211c0.307-0.896,0.469-1.843,0.469-2.79
                v-59.733c0-4.71-3.814-8.533-8.533-8.533s-8.533,3.823-8.533,8.533v58.155c-1.476,3.618-6.605,14.771-17.067,25.779V217.019
                l114.185-46.993c3.2-1.314,5.282-4.437,5.282-7.893v-8.533C512,148.889,508.186,145.066,503.467,145.066z M256,136.532
                c-10.419,0-17.067-5.052-17.067-8.533c0-3.482,6.647-8.533,17.067-8.533s17.067,5.052,17.067,8.533
                C273.067,131.481,266.419,136.532,256,136.532z M384,426.666c-4.71,0-8.533-3.831-8.533-8.533c0-4.702,3.823-8.533,8.533-8.533
                s8.533,3.831,8.533,8.533C392.533,422.834,388.71,426.666,384,426.666z" fill="#007bff"/>
                    <path d="M417.877,450.192c-1.143-4.565-5.786-7.356-10.342-6.204c-4.574,1.143-7.356,5.777-6.212,10.351l8.533,34.133
                c0.973,3.874,4.446,6.46,8.269,6.46c0.683,0,1.382-0.077,2.074-0.256c4.574-1.143,7.356-5.769,6.212-10.351L417.877,450.192z"
                          fill="#007bff"/>
</svg>
                <h4 class="h4 mb-0">Education</h4>
            </div>
        </div>
        <div class="card-body px-0 py-4">
            <div class="row justify-content-center">
                <div class="col-11">
                    @foreach($args['education'] as $education)
                        <?php
                        $location = $education['city'];
                        if ($education['region'] != "") {
                            $location .= "," . $education['region'];
                        }
                        if ($education['country'] != "") {
                            $location .= "," . $education['country'];
                        }
                        ?>
                        <div class="card mb-3 text-left">
                            <div class="card-header">
                                <h6 class="h6 mb-0">January {{ $education['year_from'] }} -
                                    January {{ $education['year_to'] }}</h6>
                            </div>
                            <div class="card-body p-0">
                                <div class="row no-gutters">
                                    <div class="col-2 text-center">
                                        <div class="px-2 py-3 h-100 border border-left-0 border-top-0 border-bottom-0">
                                            <div class="rounded p-1 bg-white d-inline-block"
                                                 style="box-shadow: 0 4px 10px 0 rgba(0, 0, 0, 0.14), 0 1px 2px 0 rgba(0, 0, 0, 0.12), 0 3px 1px -2px rgba(0, 0, 0, 0.2);">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="60px" height="60px"
                                                     enable-background="new 0 0 800 261.953" xml:space="preserve"
                                                     viewBox="0 0 201.99998 67.488425" version="1.1" y="0px" x="0px">
                                                                                        <path fill="#f40009"
                                                                                              d="m168.12 56.832c-0.003 0-0.6185 0.54875-0.6185 0.54875-0.8795 0.78625-1.7845 1.628-2.877 1.207-0.30175-0.10975-0.514-0.51175-0.565-0.82375-0.0497-2.2505 0.96375-4.3358 1.9465-6.3662l0.252-0.531c2.827-4.7202 6.113-10.282 10.939-14.142 0.81325-0.58675 1.6958-1.0798 2.6272-0.69525 0.21425 0.20075 0.46375 0.54875 0.46375 0.896 0 0.11025-0.0977 0.40275-0.15425 0.49375-1.3478 2.1782-2.6315 4.465-3.8798 6.6602-2.4395 4.3365-4.9702 8.8185-8.134 12.753zm-26.149-11.124c-0.18475 0.12875-3.445-0.9695-4.075-3.9882-0.5345-2.599 1.2552-4.6298 2.9868-5.6535 0.74325-0.58575 1.8628-0.7865 2.684-0.495 0.76225 0.513 0.96875 1.4285 0.96875 2.415 0 0.605-0.0802 1.2265-0.151 1.7935 0 0-0.0135 0.11025-0.0157 0.1285-0.54325 2.0302-1.3745 4.0248-2.3978 5.7998zm-12.454 12.277c-0.3325-0.494-0.44275-1.0342-0.45375-1.6182-0.0638-3.5875 3.796-10.012 6.5205-13.512h0.006c1.1418 2.5 3.7272 4.107 5.288 4.7842-1.9385 4.3182-8.5868 13.566-11.36 10.346zm47.054 0.637c-0.64525 0.439-1.5518 0.1015-1.2128-0.9045 0.84425-2.5615 4.2008-7.7815 4.2008-7.7815l9.349-16.582h-6.404c-0.30425 0.5-0.95525 1.6482-0.95525 1.6482-0.29625-0.4935-1.1822-1.591-1.547-1.828-1.5645-0.98875-3.86-0.52125-5.4435 0.375-6.8848 4.0258-11.908 11.98-15.87 18.182 0 0-4.1702 6.895-6.4852 7.4078-1.8072 0.1465-1.6208-2.288-1.5425-2.8552 0.69875-4.099 2.3215-7.9958 3.9565-11.527 3.3252-2.3052 7.1202-5.324 10.569-8.6355 7.4845-7.1545 13.794-15.26 14.716-17.071 0 0-0.99825 0.2015-2.177 0.23775-5.758 8.032-17.541 19.706-21.113 21.443 1.5825-3.8062 11.807-21.974 20.508-30.664l1.3692-1.3178c2.117-2.0675 4.3108-4.1898 6.0202-4.5375 0.1805-0.0182 0.413 0 0.61375 0.4215 0.0763 1.6098-0.523 2.7992-1.2105 4.153l-0.9635 1.9763s1.3648-0.25625 2.261-0.54975c1.04-1.9568 2.1918-4.153 1.7672-6.7688-0.14175-0.84174-0.751-1.482-1.5472-1.6285-2.5955-0.494-5.4242 1.4265-7.7002 2.9822l-0.092 0.0545c-11.7 9.1665-21.58 22.377-30.218 40.417-0.626 0.475-3.0448 1.0428-3.456 0.768 0.853-1.756 1.927-4.1168 2.4645-6.6782 0.093-0.732 0.1795-1.4818 0.1795-2.2135 0-1.5748-0.378-3.0562-1.7888-4.0805-1.654-0.8235-3.6982-0.439-5.0765 0.18275-6.1745 2.5615-10.789 8.8925-14.119 14.106-1.791 3.3125-3.5768 6.8602-4.0712 10.923-0.34825 3.183 0.36075 5.2322 2.1705 6.2572 1.8612 0.95075 4.2038-0.0375 5.124-0.51275 6.2552-3.2932 10.626-9.8978 14.071-15.936 0.137-0.018 2.018-0.0725 3.4408-0.439 0.019 0 0.027 0 0.0585 0.0192-0.0815 0.27325-0.81675 2.4695-0.81675 2.4695-2.1835 6.4032-3.1932 10.758-1.1308 13.338 2.9472 3.6412 7.8265-0.1275 11.796-5.105-0.852 5.9645 2.2392 6.898 4.5955 6.441 2.7068-0.7315 5.723-3.623 7.1228-5.1045-0.4495 1.7378-0.32725 4.8485 2.135 5.1592 1.6878 0.312 2.9508-0.606 4.4122-1.393 5.2422-2.8545 11.518-12.366 13.147-15.116h-2.1362c-2.316 3.5002-5.2632 8.0895-8.9705 10.268zm-80.761-35.518h5.9998l3.3862-5.5h-6.001zm95.841-14.611c-4.5748 2.5985-9.317 4.2813-15.058 3.696-1.591 1.7935-3.14 3.6412-4.5678 5.5438 8.0572 2.0492 16.599-2.5065 21.363-6.8422 5.0395-4.373 7.6238-9.8797 7.6238-9.8797s-3.7775 4.3175-9.3612 7.4822zm-50.45-1.39c-0.954 10.941-9.4588 17.364-11.563 17.875-1.2752 0.25575-3.4512-0.311-1.5088-4.9028 2.8608-6.1848 7.7448-11.435 12.985-14.125 0.1275 0.4575 0.12425 0.7495 0.0867 1.1525zm-16.572 27.572c-0.60925-1.317-2.0558-2.1588-3.5972-2.0675-5.0058 0.45775-9.902 4.5742-12.473 10.484-1.349 3.0375-2.0852 5.5438-2.568 9.5322 1.5565-1.7925 4.7315-4.738 8.3345-6.3482 0 0 0.49025-3.8238 2.9902-7.227 0.946-1.4092 2.8392-3.6778 4.965-3.0742 1.8572 0.6225 1.2045 5.7638-1.264 10.739-1.8265 3.6598-4.6215 7.32-7.4495 9.8442-2.5002 2.1225-6.1995 4.6838-9.436 2.708-2.0385-1.207-3.0782-3.7325-2.8585-6.9892 0.974-9.2392 5.1798-17.107 11.198-26.072 6.206-8.3248 13.047-16.869 22.263-21.426 1.8798-0.95125 3.6205-1.1708 5.1252-0.58525 0 0-8.651 4.757-12.775 13.558-1.049 2.2325-2.508 5.2505-1.0388 7.922 0.76775 1.3902 2.131 1.5002 3.2285 1.427 5.0548-1.1712 8.3205-5.8915 10.989-10.209 1.547-3.275 2.922-6.5318 2.922-10.155 0-0.439-0.0228-1.0242-0.06-1.4638 2.415-1.2802 7.367 0.988 7.367 0.988 3.8698 1.2812 12.11 7.5938 14.889 8.8008 1.3558-1.5732 3.5838-3.9515 4.8382-5.141l-1.816-1.1162c-2.9075-1.7748-5.9808-3.4582-9.0058-5.123-6.8698-3.7502-12.462-2.4332-15.308-1.4085-1.1185 0.40225-2.1408 0.75-2.1408 0.75-2.112-2.3052-5.5758-2.0862-8.128-1.4637-9.1785 2.6345-17.7 9.0562-26.836 20.199-6.717 8.7275-10.884 16.266-13.104 23.712-1.7118 5.0128-2.232 12.386 1.946 16.759 3.5522 3.713 8.2625 2.9088 11.589 1.5735 7.2068-3.5495 13.684-11.198 16.912-19.998 0.77875-2.616 1.6432-6.3118 0.30225-9.1298zm-79.885-8.4888c-0.012 0.0365-1.224 2.8172-1.224 2.8172-0.17525 0.14625-0.45225 0.0733-0.76975 0l-0.544-0.092c-1.7082-0.53025-2.9968-1.6648-3.394-3.0185-0.548-2.6348 1.7028-4.7022 2.6945-5.452 0.9515-0.65925 2.4308-1.0068 3.2898-0.3295 0.52975 0.62225 0.7305 1.427 0.7305 2.3058 0.00025 1.189-0.36675 2.5245-0.783 3.769zm-2.6035 5.287s-0.055 0.16525-0.0822 0.21925c-0.007 0-2.755 4.482-2.755 4.482-1.6585 2.2142-3.7292 4.996-6.3178 6.331-0.76725 0.25625-1.8278 0.42125-2.3682-0.25525-1.1458-1.3725-0.55525-3.2752-0.0755-4.812l0.16975-0.549c1.3755-3.732 3.558-7.026 5.6822-10.082 0.0265-0.018 0.086-0.0362 0.1075-0.0362 0.005 0.0183 0.0135 0.0183 0.0172 0.055 1.333 2.561 3.8325 3.7498 5.7018 4.2995 0.0113 0 0.0233 0.0182 0.0233 0.055 0.00025 0.0545-0.0303 0.12825-0.103 0.29225zm34.376-2.9455c1.2292-1.7012 4.807-6.2572 5.6792-7.0255 2.9335-2.5808 4.106-1.4452 4.2075-0.64075-2.7378 4.9032-5.969 10.52-9.007 15.57 0.006-0.018-0.95075 1.4452-0.95075 1.4452-1.3418 2.0488-2.6928 3.8052-4.8965 5.031-0.31425 0.092-0.83225 0.12875-1.177-0.12825-0.415-0.2745-0.57875-0.732-0.532-1.208 0.132-1.573 1.7338-6.384 6.6765-13.044zm-30.465-11.124c-3.7925-2.4882-11.415 2.2688-17.52 10.428-5.583 7.3192-7.9938 15.826-5.3702 19.76 3.9028 4.6288 11.148-2.104 14.229-5.7078l0.3975-0.42125c2.0952-2.2505 3.714-4.9585 5.2808-7.5568 0 0 1.44-2.3785 1.5088-2.4885 0.89-0.164 1.962-0.42025 3.17-0.768-0.0172 0.0545-4.6218 7.794-4.152 11.636 0.14625 1.172 0.0345 5.4532 4.2145 6.752 5.571 0.93375 10.02-3.3848 13.944-7.227 0 0 0.66875-0.63975 1.047-1.005-0.0957 0.38325-0.2205 0.8945-0.2205 0.8945-1.631 5.672 0.5675 6.862 1.9428 7.3012 4.1562 1.2082 9.1215-4.83 9.1375-4.83-0.12875 1.902-0.50525 3.4755 1.4998 4.738 1.8925 0.6775 3.8078-0.35525 5.2635-1.325 5.2438-3.6958 9.4682-9.48 12.88-14.48h-2.1765c-0.0257 0-5.3708 7.747-8.8415 9.4308-0.005 0-0.637 0.32425-1.001 0.0677-0.4505-0.3845-0.27275-1.1835 0.005-1.7495 0.0175-0.037 13.401-23.249 13.401-23.249h-6.319s-0.69025 1.1168-0.7465 1.19c-0.0425-0.0365-0.14375-0.2285-0.214-0.3205-3.915-5.4155-12.781 2.9145-19.456 12.574-2.5698 3.7145-5.8415 7.8968-9.234 10.916 0 0-5.0258 4.607-7.1732 1.297-0.785-1.3908-0.5435-3.424-0.10075-4.7592 2.5002-7.1172 6.8085-13.576 11.711-17.474 1.384-1.0788 2.8882-1.3908 3.6348-0.93325 0.71125 0.4215 0.826 1.4635 0.376 2.1772-1.1418-0.0365-2.0565 0.3105-2.8002 1.079-1.465 1.537-1.974 3.0192-1.5082 4.41 2.2628 3.22 6.7755-3.1472 6.5572-7.026-0.078-1.3908-0.82-2.7078-1.9272-3.3488-1.6502-0.9875-4.1678-0.73175-5.797 0.0555-2.181 0.823-5.6335 3.6952-7.6488 5.9278-2.537 2.7998-6.888 5.9098-8.2488 5.5615 0.44575-1.1898 4.121-8.7092 0.25475-11.526zm31.762 40.709c-7.442-4.794-17.542-5.6352-33.83-1.1162-17.364 4.0805-23.154 6.6962-30.757 1.4452-2.921-2.5795-3.9683-6.7328-3.1862-12.715 1.8138-9.7525 6.7152-19.138 15.422-29.548 4.8598-5.4155 9.3675-10.063 15.32-11.929 4.562-1.1708 4.1355 2.4145 3.5835 2.9272-0.5825 0-1.561 0.0918-2.3172 0.58525-0.61375 0.476-2.2985 2.013-2.389 4.0805-0.1305 3.165 3.145 2.5062 4.5578 0.7685 1.5228-1.9582 3.7732-5.7083 2.003-9.2033-0.742-1.244-2.024-2.1037-3.6158-2.3965-5.4505-0.3115-10.527 2.2867-15.212 5.1592-10.243 7.117-18.439 16.979-23.076 27.774-2.6738 6.5132-5.0918 15.843-1.4672 23.437 2.8038 5.269 8.6248 8.0678 15.57 7.4825 4.8708-0.51175 10.76-2.086 14.722-3.0365 3.9612-0.952 24.238-7.9222 30.901 4.209 0 0 2.2155-4.3008 7.7522-4.3925 4.552-0.42 11.119 1.3178 15.901 4.83-1.5932-2.396-6.1102-5.9272-9.8785-8.361z"/>
                                                                                    </svg>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-10">
                                        <div class="p-3">
                                            <a href="#" class="h5 font-weight-bold mb-0"
                                               style="opacity: 0.8; font-family: 'Open Sans', sans-serif; letter-spacing: -1px">{{ $education['school_name'] }}</a>
                                            <div class="d-flex align-items-center">
                                                <div class="mr-2">
                                            <span class="item-location-flag bfh-flag-{{ $education['country_code'] or "" }}"><i
                                                        class="glyphicon"></i> </span>
                                                </div>
                                                <p class="mb-0" style="opacity: 0.8;">
                                                    {{ $location }}</p>
                                            </div>
                                            <p class="mb-0">{{ $education['study'] }}</p>
                                            <p class="mb-0 font-weight-bold"
                                               style="opacity: 0.8; font-size: 15px;">
                                                {{ $education['grade'] or "" }}</p>
                                            <p class="mb-0"
                                               style="opacity: 0.8; font-size: 15px;">
                                                {{ $education['degree'] or "" }}</p>
                                        </div>
                                        @if(!empty($education['description']))
                                            <div class="p-3 border border-right-0 border-left-0 border-bottom-0">
                                                <pre class="mb-0">{{ $education['description'] or "" }}</pre>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                    @endforeach
                </div>
            </div>
        </div>
    </div>
@endif
@if(count($args['certification']) > 0 || count($args['distinction']) > 0)
    <div class="card mb-5">
        <div class="card-header pb-4 pt-0">
            <p class="mb-0 text-right"><a href="/user/resume/create?tab=certifications" class="text-muted"
                                          style="font-size: 14px;">Edit <img
                            src="{{ asset('img/edit.png') }}"> </a></p>
            <div>
                <svg version="1.1" class="d-block mx-auto mb-1" width="50px" height="50px" id="Layer_1"
                     xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                     viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
        <path d="M488.468,144.291H291.991l11.544-103.898c0.235-2.12-0.442-4.24-1.865-5.83c-1.423-1.589-3.455-2.498-5.589-2.498h-80.162
            c-2.134,0-4.166,0.909-5.589,2.498c-1.423,1.59-2.101,3.71-1.865,5.83l11.544,103.898H23.532C10.557,144.291,0,154.846,0,167.823
            v288.581c0,12.976,10.557,23.532,23.532,23.532h464.936c12.976,0,23.532-10.557,23.532-23.532V167.823
            C512,154.846,501.443,144.291,488.468,144.291z M287.701,47.064l-14.314,128.831c-0.029,0.262-0.249,0.459-0.513,0.459h-33.748
            c-0.264,0-0.483-0.197-0.513-0.459L224.299,47.064H287.701z M497,456.403L497,456.403c0,4.705-3.827,8.532-8.532,8.532H23.532
            c-4.705,0-8.532-3.828-8.532-8.532v-288.58c0-4.705,3.827-8.532,8.532-8.532h198.144l2.029,18.262
            c0.874,7.869,7.504,13.802,15.421,13.802h33.748c7.917,0,14.547-5.934,15.421-13.803l2.029-18.262h198.144
            c4.705,0,8.532,3.828,8.532,8.532V456.403z" fill="#007bff"/>
                    <path d="M207.903,208.419H63.613c-8.556,0-15.517,6.96-15.517,15.516v192.387c0,8.556,6.961,15.516,15.517,15.516h144.29
            c8.556,0,15.516-6.96,15.516-15.516V223.935C223.419,215.38,216.459,208.419,207.903,208.419z M176.357,416.839H95.161v-46.721
            c0-3.252,1.807-6.176,4.717-7.631l13.023-6.512l11.887,11.886c3.024,3.024,6.998,4.537,10.971,4.537
            c3.974,0,7.947-1.513,10.973-4.537l11.886-11.886l13.023,6.512c2.909,1.455,4.716,4.378,4.716,7.631V416.839z M146.375,347.001
            l-10.251,10.251c-0.2,0.201-0.528,0.203-0.73,0L125.142,347c1.339-2.314,2.084-4.977,2.084-7.78v-4.514
            c2.749,0.608,5.602,0.938,8.532,0.938s5.782-0.33,8.532-0.937v4.514C144.29,342.023,145.036,344.688,146.375,347.001z
             M135.758,320.644c-13.545,0-24.564-11.02-24.564-24.564v-16.032c0-9.125,7.424-16.548,16.549-16.548h16.032
            c9.124,0,16.548,7.423,16.548,16.548v16.032C160.322,309.625,149.303,320.644,135.758,320.644z M208.419,416.323
            c0,0.285-0.231,0.516-0.516,0.516h-16.549v-46.721c0-8.971-4.984-17.036-13.007-21.047l-18.771-9.386
            c-0.177-0.088-0.286-0.265-0.286-0.462v-11.078c0-0.092-0.01-0.181-0.014-0.272c9.728-7.215,16.046-18.779,16.046-31.793v-16.032
            c0-17.396-14.152-31.548-31.548-31.548h-16.032c-17.396,0-31.549,14.152-31.549,31.548v16.032
            c0,13.014,6.318,24.578,16.046,31.793c-0.003,0.091-0.014,0.18-0.014,0.272v11.078c0,0.197-0.109,0.374-0.285,0.462l-18.77,9.385
            c-8.024,4.011-13.009,12.076-13.009,21.047v46.721H63.613c-0.285,0-0.517-0.231-0.517-0.516V223.935
            c0-0.285,0.231-0.516,0.517-0.516h144.29c0.284,0,0.516,0.231,0.516,0.516V416.323z" fill="#007bff"/>
                    <path d="M448.387,208.419H272.032c-8.556,0-15.516,6.96-15.516,15.516v16.032c0,8.556,6.96,15.516,15.516,15.516h176.354
            c8.556,0,15.517-6.96,15.517-15.516v-16.032C463.903,215.38,456.942,208.419,448.387,208.419z M448.903,239.968
            c0,0.285-0.231,0.516-0.517,0.516H272.032c-0.284,0-0.516-0.231-0.516-0.516v-16.032c0-0.285,0.231-0.516,0.516-0.516h176.354
            c0.285,0,0.517,0.231,0.517,0.516V239.968z" fill="#007bff"/>
                    <path d="M400.29,272.547H272.032c-8.556,0-15.516,6.96-15.516,15.516v16.032c0,8.556,6.96,15.516,15.516,15.516H400.29
            c8.556,0,15.517-6.96,15.517-15.516v-16.032C415.807,279.508,408.846,272.547,400.29,272.547z M400.807,304.096
            c0,0.285-0.231,0.516-0.517,0.516H272.032c-0.284,0-0.516-0.231-0.516-0.516v-16.032c0-0.285,0.231-0.516,0.516-0.516H400.29
            c0.285,0,0.517,0.231,0.517,0.516V304.096z" fill="#007bff"/>
                    <path d="M344.178,336.677h-80.161c-4.143,0-7.5,3.358-7.5,7.5c0,4.142,3.357,7.5,7.5,7.5h80.161c4.143,0,7.5-3.358,7.5-7.5
            C351.678,340.034,348.321,336.677,344.178,336.677z" fill="#007bff"/>
                    <path d="M408.307,336.677h-32.064c-4.143,0-7.5,3.358-7.5,7.5c0,4.142,3.357,7.5,7.5,7.5h32.064c4.143,0,7.5-3.358,7.5-7.5
            C415.807,340.034,412.449,336.677,408.307,336.677z" fill="#007bff"/>
                    <path d="M280.049,384.773h-16.032c-4.143,0-7.5,3.358-7.5,7.5c0,4.142,3.357,7.5,7.5,7.5h16.032c4.143,0,7.5-3.358,7.5-7.5
            C287.549,388.131,284.191,384.773,280.049,384.773z" fill="#007bff"/>
                    <path d="M376.242,384.773h-64.129c-4.143,0-7.5,3.358-7.5,7.5c0,4.142,3.357,7.5,7.5,7.5h64.129c4.143,0,7.5-3.358,7.5-7.5
            C383.742,388.131,380.385,384.773,376.242,384.773z" fill="#007bff"/>
                    <path d="M280.049,416.839h-16.032c-4.143,0-7.5,3.358-7.5,7.5c0,4.142,3.357,7.5,7.5,7.5h16.032c4.143,0,7.5-3.358,7.5-7.5
            C287.549,420.196,284.191,416.839,280.049,416.839z" fill="#007bff"/>
                    <path d="M376.242,416.839h-64.129c-4.143,0-7.5,3.358-7.5,7.5c0,4.142,3.357,7.5,7.5,7.5h64.129c4.143,0,7.5-3.358,7.5-7.5
            C383.742,420.196,380.385,416.839,376.242,416.839z" fill="#007bff"/>
                    <path d="M256.04,144.291h-0.08c-4.143,0-7.46,3.358-7.46,7.5c0,4.142,3.397,7.5,7.54,7.5s7.5-3.358,7.5-7.5
            C263.54,147.648,260.183,144.291,256.04,144.291z" fill="#007bff"/>
</svg>
                <h4 class="h4 mb-0">Certifications & Licences</h4>
            </div>
        </div>
        <div class="card-body px-0 py-4">
            <div class="row justify-content-center">
                <div class="col-11">
                    @if(count($args['certification']) > 0)
                        <div class="mb-4">
                            <div class="row justify-content-center">
                                <div class="col-12">
                                    <h5 class="h5 mb-4">Permit / Certification / License name</h5>
                                    <div class="row">
                                        @foreach($args['certification'] as $certification)
                                            <div class="col-6">
                                                <div class="mb-3 border rounded p-2 bg-light">
                                                    <div class="text-left">
                                                        <p class="mb-0">{{ $certification['title'] }}
                                                            <span class="text-secondary">{{ ucfirst($certification['type']) }}</span>
                                                            <span class="font-weight-bold">{{ $certification['year'] }}</span>
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                        </div>
                    @endif
                    @if(count($args['distinction']) > 0)
                        <div>
                            <div class="row justify-content-center">
                                <div class="col-12">
                                    <h5 class="h5 mb-4">Distinctions / Outstanding achievements</h5>
                                    <div class="row">
                                        @foreach($args['distinction'] as $distinction)
                                            <div class="col-6">
                                                <div class="mb-3 border rounded p-2 bg-light">
                                                    <div class="text-left">
                                                        <p class="mb-0">{{ $distinction['title'] }}
                                                            <span class="font-weight-bold">{{ $distinction['year'] }}</span>
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                        </div>
                    @endif
                </div>
            </div>
        </div>
    </div>
@endif
@if(count($args['skill']) > 0 || count($args['languages']) > 0)
    <div class="card mb-5">
        <div class="card-header pb-4 pt-0">
            <p class="mb-0 text-right"><a href="/user/resume/create?tab=skills" class="text-muted"
                                          style="font-size: 14px;">Edit <img
                            src="{{ asset('img/edit.png') }}"> </a></p>
            <div>
                <svg version="1.1" id="Layer_1" class="d-block mx-auto mb-1" xmlns="http://www.w3.org/2000/svg"
                     width="50px"
                     height="50px" x="0px" y="0px"
                     viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
            <path d="M373.44,318.229c0.981-2.304,2.027-4.331,3.243-5.547c2.88-2.859,9.643-4.907,16.192-6.891
                c10.88-3.307,23.232-7.04,29.44-17.771c6.123-10.581,3.221-23.04,0.683-34.027c-1.579-6.741-3.2-13.717-2.091-17.792
                c0.96-3.733,5.931-9.003,10.304-13.653C439.083,214.165,448,204.672,448,192s-8.917-22.165-16.789-30.549
                c-4.373-4.651-9.323-9.92-10.304-13.632c-1.088-4.075,0.533-11.072,2.091-17.813c2.539-10.987,5.44-23.445-0.683-34.048
                c-6.208-10.731-18.539-14.464-29.44-17.749c-6.549-1.984-13.291-4.032-16.171-6.891c-2.88-2.859-4.907-9.621-6.891-16.171
                c-3.285-10.88-7.019-23.211-17.749-29.419c-10.645-6.165-23.061-3.264-34.069-0.704c-6.741,1.557-13.696,3.179-17.792,2.091
                c-3.733-1.003-9.003-5.952-13.653-10.325C278.166,8.917,268.672,0,256,0c-12.651,0-22.144,8.917-30.528,16.789
                c-4.672,4.373-9.941,9.323-13.675,10.325c-4.096,1.109-11.072-0.533-17.792-2.091c-11.029-2.56-23.467-5.44-34.069,0.683
                c-10.709,6.208-14.443,18.539-17.728,29.44c-1.984,6.549-4.032,13.312-6.891,16.171c-2.88,2.859-9.643,4.907-16.192,6.891
                c-10.88,3.307-23.232,7.04-29.44,17.771c-6.123,10.581-3.221,23.04-0.683,34.027c1.579,6.741,3.2,13.717,2.091,17.792
                c-0.96,3.733-5.931,9.003-10.304,13.653C72.918,169.835,64,179.349,64,192s8.917,22.165,16.789,30.549
                c4.373,4.651,9.323,9.92,10.304,13.632c1.088,4.075-0.533,11.072-2.091,17.813c-2.539,10.987-5.44,23.445,0.683,34.048
                c6.208,10.731,18.539,14.464,29.44,17.749c6.549,1.984,13.291,4.032,16.171,6.891c1.216,1.216,2.261,3.221,3.243,5.525
                L64.811,497.259c-1.707,4.117-0.661,8.853,2.581,11.883c2.027,1.877,4.629,2.859,7.275,2.859c1.621,0,3.243-0.363,4.779-1.152
                l79.787-39.893l58.859,39.232c2.709,1.792,6.101,2.24,9.195,1.28c3.093-1.003,5.568-3.349,6.699-6.4l23.296-62.123l20.587,61.76
                c1.067,3.157,3.541,5.632,6.677,6.72c3.136,1.067,6.592,0.64,9.365-1.216l58.859-39.232l79.787,39.893
                c1.536,0.768,3.157,1.131,4.779,1.131c2.645,0,5.248-0.981,7.253-2.837c3.243-3.029,4.288-7.765,2.581-11.883L373.44,318.229z
                 M246.016,412.224l-27.285,72.747l-52.821-35.2c-3.221-2.112-7.296-2.389-10.688-0.661L95.36,479.04l54.229-131.733
                c2.56,4.352,5.717,8.277,10.347,10.965c10.645,6.165,23.083,3.264,34.069,0.704c6.72-1.557,13.675-3.221,17.792-2.091
                c3.733,1.003,9.003,5.952,13.653,10.325c3.008,2.816,6.229,5.653,9.6,8.277c0.085,0.405,0.021,0.811,0.149,1.216l11.477,34.453
                C246.486,411.541,246.166,411.819,246.016,412.224z M255.979,362.667c-4.203,0-10.432-5.845-15.936-11.008
                c-6.784-6.4-13.824-12.992-22.741-15.381c-2.88-0.768-5.781-1.088-8.704-1.088c-6.464,0-13.013,1.536-19.392,3.008
                c-6.827,1.6-15.275,3.563-18.56,1.621c-3.435-1.984-5.867-10.027-8.021-17.131c-2.731-9.067-5.547-18.432-12.224-25.088
                c-6.656-6.656-16.021-9.493-25.088-12.224c-7.104-2.133-15.147-4.565-17.152-8c-1.899-3.307,0.064-11.755,1.643-18.56
                c2.155-9.301,4.373-18.901,1.92-28.139c-2.411-8.917-9.003-15.936-15.381-22.741c-5.184-5.504-11.029-11.733-11.029-15.936
                s5.867-10.432,11.029-15.936c6.379-6.784,12.992-13.824,15.381-22.741c2.453-9.216,0.235-18.816-1.92-28.117
                c-1.579-6.805-3.541-15.253-1.643-18.539c2.005-3.435,10.048-5.867,17.173-8.021c9.067-2.731,18.411-5.568,25.088-12.224
                c6.635-6.635,9.472-16.021,12.224-25.067c2.155-7.104,4.587-15.168,8-17.152c3.349-1.877,11.776,0.064,18.539,1.643
                c9.259,2.155,18.837,4.395,28.117,1.92c8.917-2.389,15.957-9.003,22.763-15.381c5.483-5.163,11.712-11.008,15.915-11.008
                c4.203,0,10.432,5.845,15.936,11.008c6.784,6.4,13.824,12.992,22.741,15.381c9.259,2.475,18.859,0.235,28.096-1.92
                c6.805-1.6,15.275-3.541,18.56-1.621c3.435,1.984,5.867,10.027,8.021,17.131c2.731,9.067,5.547,18.432,12.224,25.088
                c6.656,6.656,16.021,9.493,25.088,12.224c7.104,2.133,15.147,4.565,17.152,8c1.899,3.307-0.064,11.755-1.643,18.56
                c-2.155,9.301-4.373,18.901-1.92,28.139c2.411,8.917,9.003,15.936,15.381,22.741c5.184,5.504,11.029,11.733,11.029,15.936
                s-5.867,10.432-11.029,15.936c-6.379,6.784-12.992,13.824-15.381,22.741c-2.453,9.216-0.235,18.816,1.92,28.117
                c1.579,6.805,3.541,15.253,1.643,18.539c-2.005,3.435-10.048,5.867-17.173,8.021c-9.067,2.731-18.411,5.568-25.088,12.224
                c-6.635,6.635-9.472,16.021-12.224,25.067c-2.155,7.104-4.587,15.168-8,17.152c-3.285,1.899-11.755-0.064-18.539-1.643
                c-9.259-2.155-18.837-4.395-28.117-1.92c-8.917,2.389-15.957,9.003-22.763,15.381
                C266.411,356.821,260.182,362.667,255.979,362.667z M356.758,449.131c-3.413-1.728-7.509-1.472-10.688,0.661l-52.373,34.923
                l-33.792-101.376c10.667-1.621,19.157-9.109,26.624-16.128c4.672-4.373,9.941-9.323,13.675-10.325
                c4.117-1.088,11.072,0.533,17.792,2.091c11.029,2.56,23.467,5.44,34.069-0.683c4.608-2.667,7.765-6.613,10.325-10.965
                l54.229,131.733L356.758,449.131z" fill="#007bff"/>
                    <path d="M384,192c0-70.592-57.408-128-128-128s-128,57.408-128,128s57.408,128,128,128S384,262.592,384,192z M256,298.667
                c-58.816,0-106.667-47.851-106.667-106.667S197.184,85.333,256,85.333S362.667,133.184,362.667,192S314.816,298.667,256,298.667z
                " fill="#007bff"/>
</svg>
                <h4 class="h4 mb-0">Skills & Languages</h4>
            </div>
        </div>
        <div class="card-body px-0 py-4">
            <div class="row justify-content-center">
                <div class="col-11">
                    @if(count($args['skill']) > 0)
                        <div class="mb-4">
                            <h5 class="h5 mb-4">Skills</h5>
                            @foreach($args['skill'] as $skill)
                                <div class="card text-left mb-3">
                                    <div class="card-header">
                                        <h6 class="h6 mb-0">{{ $skill['title'] }}</h6>
                                    </div>
                                    <div class="card-body d-flex justify-content-between align-items-baseline">
                                        <pre class="mb-0" style="flex: 1">{{ $skill['description'] }}</pre>
                                        <div class="bg-light rounded border"
                                             style="width:130px; height: 12px">
                                            <div class="bg-primary rounded"
                                                 style="width: {{ $skill['level'] }}%; height: 100%; opacity: 0.8"></div>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    @endif
                    @if(count($args['languages']) > 0)
                        <div>
                            <div class="row justify-content-center">
                                <div class="col-12">
                                    <h5 class="h5 mb-4">Languages</h5>
                                    <div class="row">
                                        @foreach($args['languages'] as $language)
                                            <div class="col-6">
                                                <div class="card text-left mb-3">
                                                    <div class="card-body d-flex justify-content-between align-items-baseline">
                                                        <p class="mb-0" style="flex: 1">{{ $language['title'] }}</p>
                                                        <div class="bg-light rounded border"
                                                             style="width:30%; height: 12px">
                                                            <div class="bg-primary rounded"
                                                                 style="width: {{ $language['level'] }}%; height: 100%; opacity: 0.8"></div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                        </div>
                    @endif
                </div>
            </div>
        </div>
    </div>
@endif
@if(count($args['interest']) > 0 || count($args['hobby']) > 0)
    <div class="card">
        <div class="card-header pb-4 pt-0">
            <p class="mb-0 text-right"><a href="/user/resume/create?tab=interest" class="text-muted"
                                          style="font-size: 14px;">Edit <img
                            src="{{ asset('img/edit.png') }}"> </a></p>
            <div>
                <svg version="1.1" id="Capa_1" class="d-block mx-auto mb-1" xmlns="http://www.w3.org/2000/svg"
                     width="50px"
                     height="50px" x="0px" y="0px"
                     viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
            <path d="M330.667,122.667c23.467,0,42.667-19.2,42.667-42.667s-19.2-42.667-42.667-42.667S288,56.533,288,80
                S307.2,122.667,330.667,122.667z" fill="#007bff"/>
                    <path d="M106.667,261.333C48,261.333,0,309.333,0,368s48,106.667,106.667,106.667c58.667,0,106.667-48,106.667-106.667
                S165.333,261.333,106.667,261.333z M106.667,442.667C65.067,442.667,32,409.6,32,368s33.067-74.667,74.667-74.667
                c41.6,0,74.667,33.067,74.667,74.667S148.267,442.667,106.667,442.667z" fill="#007bff"/>
                    <path d="M404.267,238.933v-42.667c-32,0-58.667-11.733-77.867-30.933L284.8,124.8c-6.4-7.467-17.067-12.8-28.8-12.8
                s-22.4,4.267-29.867,12.8l-58.667,58.667c-7.467,7.467-12.8,18.133-12.8,29.867s5.333,22.4,12.8,30.933l67.2,59.733v106.667
                h42.667V277.333L230.4,230.4l50.133-51.2l16,16C323.2,222.933,360.533,238.933,404.267,238.933z"
                          fill="#007bff"/>
                    <path d="M405.333,261.333c-58.667,0-106.667,48-106.667,106.667s48,106.667,106.667,106.667C464,474.667,512,426.667,512,368
                S464,261.333,405.333,261.333z M405.333,442.667c-41.6,0-74.667-33.067-74.667-74.667s33.067-74.667,74.667-74.667
                C446.933,293.333,480,326.4,480,368S446.933,442.667,405.333,442.667z" fill="#007bff"/>
</svg>
                <h4 class="h4 mb-0">Hobbies & Interests</h4>
            </div>
        </div>
        <div class="card-body px-0 py-4">
            <div class="row justify-content-center">
                <div class="col-11">
                    @if(count($args['hobby']) > 0)
                        <div class="mb-4">
                            <h5 class="h5 mb-4">Hobbies</h5>
                            @foreach($args['hobby'] as $hobby)
                            <div class="card text-left mb-3">
                                <div class="card-header">
                                    <h6 class="h6 mb-0">{{ $hobby['title'] }}</h6>
                                </div>
                                <div class="card-body">
                                    <pre class="mb-0">{{ $hobby['description'] }}</pre>
                                </div>
                            </div>
                            @endforeach
                        </div>
                    @endif
                    @if(count($args['interest']) > 0)
                        <div>
                            <h5 class="h5 mb-4">Interests</h5>
                            @foreach($args['interest'] as $interest)
                            <div class="card text-left mb-3">
                                <div class="card-header">
                                    <h6 class="h6 mb-0">{{ $interest['title'] }}</h6>
                                </div>
                                <div class="card-body">
                                    <pre class="mb-0">{{ $interest['description'] }}</pre>
                                </div>
                            </div>
                            @endforeach
                        </div>
                    @endif
                </div>
            </div>
        </div>
    </div>
@endif