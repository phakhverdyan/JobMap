...
<div class="card border-0 col-md-12 px-0">
    <div class="card-header bg-white border-0 py-3 BobikHoverEffect text-left" role="tab" id="heading{{ $args['id'] }}">
        <h5 class="mb-0 mt-0">
            ...
            <div class="float-right collapsed Bobik mt-1" id="accord_buttonH{{ $args['id'] }}"></div>

        </h5>
    </div>

    <div id="collapse{{ $args['id'] }}" class="text-left"
         aria-labelledby="heading{{ $args['id'] }}" style="display: none;">
...
$(document).ready(
    function(){
        $('#accord_buttonH{{ $args['id'] }}').click(function(){
            $('#collapse{{ $args['id'] }}').slideToggle();
            $('#accord_buttonH{{ $args['id'] }}').toggleClass('collapsed');
            return false;
        });
    });
</script>
