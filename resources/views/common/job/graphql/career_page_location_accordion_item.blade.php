...
<div class="card border-0 col-md-12 px-0">
    <div class="card-header bg-white border-0 py-3 BobikHoverEffect text-left" role="tab" id="headingL{{ $args['id'] }}">
        <h5 class="mb-0 mt-0">
                                    <span>
                                        <img src="{{ $picture }}" style="width: 35px;" class="mr-2">
                                    </span>
            <a href="{{ url('/map/view/location/' . $args['id']) }}" style="font-size: 16px; font-family: sans-regular; color: #4E5C6E;">

                {{ $args['name'] }}
            </a>

            <div class="float-right collapsed Bobik" id="accord_buttonL{{ $args['id'] }}"></div>

        </h5>
    </div>

    <div id="collapseL{{ $args['id'] }}" class="text-left accordion_effect" aria-labelledby="headingL{{ $args['id'] }}" style="display: none;">
...
$(document).ready(
    function(){
        $('#accord_buttonL{{ $args['id'] }}').click(function(){
            $('#collapseL{{ $args['id'] }}').slideToggle();
            $('#accord_buttonL{{ $args['id'] }}').toggleClass('collapsed');
            return false;
        });
    });
</script>
