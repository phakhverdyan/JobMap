...
@section('content')
    ...
    <div id="location-view" style="margin-top: 55px;">
        ...
        <div class="text-center py-4"
             style="background-color: rgba(0,0,0,0.6); position: absolute; width: 100%; z-index: 1;">
            <p class="text-center mb-0">
                <a href="/map/world" id="link-location-world" class="text-white" data-toggle="tooltip"
                   data-placement="top" title="{!! trans('main.explore_the_world') !!}">{!! trans('main.world') !!}</a>
                <span style="opacity: 0.2;">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                         id="Capa_1" x="0px" y="0px" width="13px" height="13px" viewBox="0 0 292.359 292.359"
                            ...
                            <div class="d-flex flex-sm-row flex-column justify-content-between justify-content-lg-start w-100">
                                <div class="text-center py-3 mxa-0 mr-5">
                                    <div class="mb-3 mb-lg-0">
                                        <a href="{{ $main_link }}?type=employers" class="business_tabs {{ $type_items == 'employers' ? 'active' : '' }} mxa-0">
                                            <span id="count_employers">{!! trans('main.counters.employers_c', ['count' => $data->count_employers]) !!}</span>
                                        </a>
                                    </div>
                                </div>
                                <div class="text-center py-3 mxa-0 mr-5">
                                    <div class="mb-3 mb-lg-0">
                                        <a href="{{ $main_link }}?type=jobs" class="business_tabs {{ $type_items == 'jobs' ? 'active' : '' }} mxa-0">
                                            <span id="count_jobs">{!! trans('main.counters.jobs_c', ['count' => $data->count_jobs]) !!}</span>
                                        </a>
                                    </div>
                                </div>
                                <div class="text-center py-3 mxa-0 mr-5">
                                    <div class="mb-3 mb-lg-0">
                                        <a href="{{ $main_link }}?type=headquarters" class="business_tabs {{ $type_items == 'headquarters' ? 'active' : '' }} mxa-0">
                                            <span id="count_headquarters">{!! trans('main.counters.headquarters_c', ['count' => $data->count_headquarters]) !!}</span>
                                        </a>
                                    </div>
                                </div>
                                <div class="text-center py-3 mxa-0">
                                    <div class="mb-3 mb-lg-0">
                                        <a href="{{ $main_link }}?type=locations" class="business_tabs {{ $type_items == 'locations' ? 'active' : '' }} mxa-0">
                                            <span id="count_locations">{!! trans('main.counters.branches_c', ['count' => $data->count_locations]) !!}</span>
                                        </a>
                                    </div>
                                </div>
