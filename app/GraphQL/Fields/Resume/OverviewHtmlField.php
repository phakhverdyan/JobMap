...
class OverviewHtmlField extends Field
{
    ...
    protected function resolve($root, $args)
    {
        ...
        $url = 'https://api.teleport.org/api/cities/?search='.$root['city'].'&embed=city:search-results/city:item/%7Bcity:country,city:admin1_division,city:timezone/tz:offsets-now,city:urban_area%7D&limit=1';
        /*$ch = curl_init($url);
        ...
        if ($resultArray->_embedded->{'city:search-results'}) {
            $res = $resultArray->_embedded->{'city:search-results'};
            $data['latitude'] = $res[0]->_embedded->{'city:item'}->location->latlon->latitude;
            $data['longitude'] = $res[0]->_embedded->{'city:item'}->location->latlon->longitude;
        } else {
            $data['latitude'] = 50;
            $data['longitude'] = 26;
        }
        ...
        $view = View('components.resume_builder.graphql.overview', [
            'args' => $root,
            'data' => $data
        ])->render();
