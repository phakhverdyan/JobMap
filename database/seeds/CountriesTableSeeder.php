...
class CountriesTableSeeder extends Seeder
{
	public function run()
	{
		...
		$country_phone_number_details = json_decode(File::get(storage_path('app/geo/country-phone-masks.json')), true);

		foreach ($country_phone_number_details as $country_code => $country_phone_number_detail) {
			Country::where('alpha2_code', $country_code)->update([
				'phone_code' => $country_phone_number_detail['phone_code'],
				'phone_number_mask' => $country_phone_number_detail['phone_number_mask'],
			]);
		}
